...
void main() {
  test('M31 start permission path does not request exact alarm access', () {
    ...
    expect(
      providers,
      isNot(contains('NotificationBootstrap.scheduler.requestPermissions')),
    );
    expect(providers, contains('SleepSafetyNotificationPermissionGateway'));
  ...
  test(
    'Android start path converts foreground-service failures to safe errors',
    () {
      final channel = File(
      ...
      expect(channel, contains('dismissAlert'));
      expect(service, contains('promoteToForegroundSafely()'));
      expect(service, contains('ACTION_DISMISS_ALERT'));

      final promotionIndex = service.indexOf(
        'if (!promoteToForegroundSafely()) return',
      );
      final activeIndex = service.indexOf(
  ...
  test(
    'AudioRecord worker catches runtime failures instead of crashing process',
    () {
      final capture = File(
      ...
      expect(
        capture,
        contains('private val failureSignaled = AtomicBoolean(false)'),
      );
