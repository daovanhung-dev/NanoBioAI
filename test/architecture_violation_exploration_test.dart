...
void main() {
  group('Bug Condition Exploration - Architecture Violations', () {
    test(
      'Violation 1: Cross-Feature Dependency - OnboardingController imports DashboardController',
      () {
        ...
        expect(
          file.existsSync(),
          isTrue,
          reason: 'OnboardingController file should exist',
        );

        final content = file.readAsStringSync();

        // Check for the cross-feature import
        final hasCrossFeatureImport =
            content.contains('import') &&
            content.contains(
              'dashboard/presentation/controllers/dashboard_controller.dart',
            );
        ...
        expect(
          hasCrossFeatureImport,
          isFalse,
          reason:
              'OnboardingController should NOT import DashboardController - violates feature independence',
        );
        ...
        expect(
          ...
          isFalse,
          reason:
              'OnboardingController should NOT directly call DashboardController - violates feature independence',
        );
      },
    );

    test(
      'Violation 2: Circular Dependency - AIService imports DashboardEntity while Dashboard imports AIService',
      () {
        ...
        expect(
          aiServiceFile.existsSync(),
          isTrue,
          reason: 'AIService file should exist',
        );
        expect(
          dashboardControllerFile.existsSync(),
          isTrue,
          reason: 'DashboardController file should exist',
        );
        ...
        final aiServiceImportsDashboard =
            aiServiceContent.contains('import') &&
        ...
        final dashboardImportsAI =
            dashboardContent.contains('import') &&
        ...
        expect(
          aiServiceImportsDashboard,
          isFalse,
          reason:
              'AIService (core service) should NOT import DashboardEntity (feature entity) - violates dependency direction',
        );
        ...
        expect(
          ...
          isTrue,
          reason:
              'DashboardController correctly imports AIService (feature depends on service)',
        );
      },
    );

    test(
      'Violation 3: Misnamed Datasource - "remote" datasource uses SQLite (local)',
      () {
        ...
        if (remoteExists) {
          ...
          expect(
            ...
            isFalse,
            reason:
                'File named "remote_datasource" should NOT use SQLite - should be renamed to "local_datasource"',
          );
        }
        ...
        expect(
          ...
          isTrue,
          reason:
              'Datasource using SQLite should be named "onboarding_local_datasource.dart"',
        );
        ...
        expect(
          ...
          isFalse,
          reason:
              'File "onboarding_remote_datasource.dart" should not exist (should be renamed to local)',
        );
      },
    );

    test(
      'Violation 4: Nested Feature Structure - meal_plan contains nested dashboard folder',
      () {
        ...
        expect(
          nestedDashboardDir.existsSync(),
          isFalse,
          reason:
              'meal_plan feature should NOT have nested "dashboard" folder - violates flat feature structure',
        );
    ...
    test(
      'Violation 5: Misplaced Model - MealPlanModel in core layer instead of feature',
      () {
        ...
        expect(
          coreModelFile.existsSync(),
          isFalse,
          reason:
              'MealPlanModel should NOT be in core layer - it is feature-specific',
        );
    ...
    test('Integration: Detect all violations together', () {
      ...
      if (onboardingFile.existsSync()) {
        ...
        if (content.contains(
          ...
          violations.add(
            'Violation 1: OnboardingController imports DashboardController (line ~4)',
          );
        }
        if (content.contains('dashboardControllerProvider')) {
          violations.add(
            'Violation 1: OnboardingController calls dashboardControllerProvider directly (line ~302)',
          );
        }
      }
      ...
      if (aiServiceFile.existsSync()) {
        ...
        if (content.contains('features/dashboard/domain/entities')) {
          violations.add(
            'Violation 2: AIService imports DashboardEntity from feature (line ~9)',
          );
        }
      }
      ...
      if (remoteDatasource.existsSync()) {
        ...
        if (content.contains('sqflite') ||
            content.contains('DatabaseService')) {
          violations.add(
            'Violation 3: onboarding_remote_datasource.dart uses SQLite but named "remote"',
          );
        }
      }
      ...
      if (File(
        ...
        violations.add(
          'Violation 5: MealPlanModel in core layer instead of meal_plan feature',
        );
      }
