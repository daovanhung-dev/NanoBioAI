...
void main() {
  test('medical design system is wired into every app surface', () {
    ...
    for (final path in appFiles) {
      ...
      expect(
        source,
        contains('AppExperience.builder'),
  ...
  test(
    'theme exposes the healthcare shell and Material 3 component system',
    () {
  ...
  test('application pages use the medical shell instead of raw Scaffold', () {
    final rawScaffoldFiles = Directory('lib')
        ...
        .where((file) {
          ...
          return !normalized.contains('/app_versions/admin/') &&
              !normalized.endsWith('/core/theme/medical_ui.dart') &&
              !normalized.endsWith('/core/theme/design_system_demo_page.dart');
        })
        .where(
          (file) =>
              RegExp(r'\bScaffold\s*\(').hasMatch(file.readAsStringSync()),
        )
        .map((file) => file.path)
        .toList();
