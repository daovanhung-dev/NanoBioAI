...
import 'package:nano_app/features/onboarding/domain/entities/onboarding_entity.dart';
  ...
  group('Preservation Properties - Functional Behavior Unchanged', () {
    ...
    group('Property 3.1: Onboarding Completion Triggers Meal Generation', () {
      test('FOR ALL onboarding completions, meal generation is triggered', () {
        ...
        expect(content.contains('Future<void> saveOnboarding()'), isTrue,
        ...
        expect(triggersGeneration, isTrue,
          reason: 'Onboarding completion must trigger meal generation (3.1)');
        
        // Verify AppPrefs.setOnboardingCompleted is still called
        expect(content.contains('AppPrefs.setOnboardingCompleted'), isTrue,
          reason: 'Must mark onboarding as completed');
      });
      
      test('Property: Onboarding state transitions preserve all 7 wizard steps', () {
        ...
        expect(content.contains('currentStep >= 6'), isTrue,
          reason: '7-step wizard structure (0-6) must be preserved');
        
        expect(content.contains('currentStep <= 0'), isTrue,
          reason: 'Step boundaries must be preserved');
      });
    });
    
    group('Property 3.2: AI Service Integration Preserved', () {
      test('FOR ALL AI service calls, Gemini API integration works identically', () {
        ...
        expect(content.contains('GenerativeModel'), isTrue,
          reason: 'Must use GenerativeModel for Gemini API');
        
        expect(content.contains('GEMINI_API_KEY'), isTrue,
          reason: 'Must use API key from environment');
        
        expect(content.contains('generateContent'), isTrue,
          reason: 'Must call generateContent API');
        
        // Verify generateMealPlan method signature exists
        expect(content.contains('Future<List<MealPlanModel>> generateMealPlan'), isTrue,
          reason: 'generateMealPlan method must exist and return List<MealPlanModel>');
        
        // Verify JSON parsing logic preserved
        expect(content.contains('jsonDecode'), isTrue,
          reason: 'JSON parsing must be preserved');
        
        expect(content.contains('MealPlanModel.fromJson'), isTrue,
      ...
      test('Property: AI service accepts health data parameter (interface may change)', () {
        ...
        expect(hasHealthDataParam, isTrue,
    ...
    group('Property 3.3: Meal Plan Database Storage Preserved', () {
      test('FOR ALL meal plan saves, SQLite database stores them correctly', () {
        ...
        final datasourceFile = File('lib/features/dashboard/data/datasources/dashboard_local_datasource.dart');
        expect(datasourceFile.existsSync(), isTrue);
        
        final content = datasourceFile.readAsStringSync();
        
        // Verify saveMealPlan method exists
        expect(content.contains('Future<void> saveMealPlan'), isTrue,
        ...
        expect(content.contains('MealPlansDao'), isTrue,
          reason: 'Must use MealPlansDao for database operations');
        
        expect(content.contains('insertMany'), isTrue,
      ...
      test('Property: MealPlanModel structure preserved (location may change)', () {
        ...
        final modelExists = coreModelFile.existsSync() || featureModelFile.existsSync();
        ...
        final requiredFields = [
          'String id', 'String planDate', 'String mealType', 'String mealName',
          'String description', 'int calories', 'double protein', 'double carbs',
          'double fat', 'double fiber', 'int waterMl', 'int mealOrder',
          'bool isCompleted', 'bool aiGenerated'
        ];
        
        for (final field in requiredFields) {
          expect(content.contains(field), isTrue,
            reason: 'Required field "$field" must be preserved in MealPlanModel');
        }
        ...
        expect(content.contains('toMap'), isTrue,
          reason: 'toMap for database must be preserved');
      });
    });
    
    group('Property 3.4 & 3.5: Health Data Storage and Retrieval Across 7 Tables', () {
      test('FOR ALL health data saves, all 7 SQLite tables receive correct records', () {
        // OBSERVATION: onboarding_remote_datasource saves to 7 tables
        // INVARIANT: Multi-table save logic must be preserved
        
        final datasourceFile = File('lib/features/onboarding/data/datasource/onboarding_remote_datasource.dart');
        final altDatasourceFile = File('lib/features/onboarding/data/datasource/onboarding_local_datasource.dart');
        
        final fileExists = datasourceFile.existsSync() || altDatasourceFile.existsSync();
        expect(fileExists, isTrue,
          reason: 'Onboarding datasource must exist (may be renamed to local)');
        ...
        expect(content.contains('survey_answers'), isTrue,
          reason: 'Must handle survey_answers table (3.4)');
      });
      
      test('FOR ALL dashboard fetches, health data is read from all tables correctly', () {
        ...
        expect(content.contains('Future<DashboardEntity> fetchDashboard'), isTrue,
        ...
        expect(content.contains('return DashboardEntity'), isTrue,
      ...
      test('Property: DashboardEntity structure preserved', () {
        ...
        final entityFile = File('lib/features/dashboard/domain/entities/dashboard_entity.dart');
        ...
        final requiredFields = [
          ...
          'String gender', 'int birthYear', 'String occupation',
          'double heightCm', 'double weightKg', 'double bmi',
          'List<String> goals', 'List<String> conditions', 'List<String> habits',
          'String sleepQuality', 'String activityLevel', 'String waterPerDay',
          'String allergyName', 'String allergyNote',
          'String treatmentName', 'String medicationName', 'String treatmentNote',
          'String concernText', 'Map<String, String> surveyAnswers'
        ];
        
        for (final field in requiredFields) {
          expect(content.contains(field), isTrue,
            reason: 'Required field "$field" must be preserved in DashboardEntity');
        }
      });
    });
    ...
    group('Property 3.6: JSON Serialization/Deserialization Preserved', () {
      test('FOR ALL meal plan models, JSON operations work identically', () {
        ...
        expect(content.contains('factory MealPlanModel.fromJson'), isTrue,
          reason: 'fromJson must be preserved for AI response parsing (3.6)');
        
        expect(content.contains('factory MealPlanModel.fromMap'), isTrue,
          reason: 'fromMap must be preserved for database reads (3.6)');
        
        expect(content.contains('Map<String, dynamic> toJson'), isTrue,
          reason: 'toJson must be preserved (3.6)');
        
        expect(content.contains('Map<String, dynamic> toMap'), isTrue,
        ...
        expect(content.contains('isCompleted') && content.contains('? 1 : 0'), isTrue,
    ...
    group('Property 3.7 & 3.8: UI/UX Navigation and Display Preserved', () {
      test('FOR ALL onboarding completions, navigation to dashboard works', () {
        ...
        expect(content.contains('AppPrefs.setOnboardingCompleted(true)'), isTrue,
          reason: 'Must mark onboarding as completed for navigation (3.7)');
        
        // Verify success/error state handling preserved
        expect(content.contains('savedLog'), isTrue,
          reason: 'UI feedback mechanism must be preserved (3.7)');
      });
      
      test('Property: Dashboard controller genMealByWeeksToDB flow preserved', () {
        // OBSERVATION: dashboard_controller.dart orchestrates meal generation
        // INVARIANT: Orchestration logic must remain unchanged
        
        final dashboardFile = File('lib/features/dashboard/presentation/controllers/dashboard_controller.dart');
        ...
        expect(content.contains('Future<void> genMealByWeeksToDB'), isTrue,
          reason: 'genMealByWeeksToDB method must exist');
        
        // Verify the orchestration steps: fetch data → AI generation → save
        expect(content.contains('fetchDashboard'), isTrue,
          reason: 'Must fetch dashboard data (3.8)');
        
        expect(content.contains('generateMealPlan'), isTrue,
          reason: 'Must call AI service to generate meal plan (3.2, 3.8)');
        
        expect(content.contains('saveMealPlan'), isTrue,
    ...
    group('Property 3.9 & 3.10: State Management Patterns Preserved', () {
      test('FOR ALL provider accesses, API surface remains unchanged', () {
        ...
        expect(onboardingContent.contains('class OnboardingController extends Notifier<OnboardingState>'), isTrue,
          reason: 'OnboardingController must extend Notifier (3.10)');
        
        expect(onboardingContent.contains('final onboardingProvider'), isTrue,
          reason: 'onboardingProvider must exist (3.9)');
        
        // Verify AsyncNotifier pattern preserved for DashboardController
        expect(dashboardContent.contains('class DashboardController extends AsyncNotifier'), isTrue,
          reason: 'DashboardController must extend AsyncNotifier (3.10)');
        
        expect(dashboardContent.contains('final dashboardControllerProvider'), isTrue,
      ...
      test('Property: Controller state management methods preserved', () {
        ...
        expect(content.contains('state.copyWith'), isTrue,
          reason: 'Immutable state update pattern must be preserved (3.10)');
      });
    });
    
    group('Integration Property: End-to-End Flow Preservation', () {
      test('Property: Complete onboarding → meal generation → dashboard flow preserved', () {
        ...
        final onboardingFile = File('lib/features/onboarding/presentation/controllers/onboarding_controller.dart');
        final onboardingContent = onboardingFile.readAsStringSync();
        expect(onboardingContent.contains('Future<void> saveOnboarding()'), isTrue,
        ...
        final dashboardFile = File('lib/features/dashboard/presentation/controllers/dashboard_controller.dart');
        final dashboardContent = dashboardFile.readAsStringSync();
        expect(dashboardContent.contains('Future<void> genMealByWeeksToDB'), isTrue,
        ...
        expect(aiServiceContent.contains('Future<List<MealPlanModel>> generateMealPlan'), isTrue,
        ...
        expect(datasourceContent.contains('Future<void> saveMealPlan'), isTrue,
