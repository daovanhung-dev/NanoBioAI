...
- docs/worklog/2026-08-24/002-worklog-admin-workspace-simplification.md :: - Nếu audit fail, cố gắng xóa bù Auth user vừa tạo và trả safe error.
- docs/worklog/2026-08-24/002-worklog-admin-workspace-simplification.md :: - Không thêm schema mới: dùng `membership_subscriptions.source = manual` đúng canonical constraint.
- docs/worklog/2026-08-24/002-worklog-admin-workspace-simplification.md :: - Provenance/idempotency dùng `provider = admin_manual` và `provider_subscription_id = idempotency_key`.
- docs/worklog/2026-08-24/002-worklog-admin-workspace-simplification.md :: Plan đề xuất có thể thêm RPC `admin_grant_membership`. Sau khi đọc source-of-truth Supabase mới, repo quy định `01..06` là authored canonical và `config.sql` là generated. Để không tạo drift schema/config trong phiên không có full local clone và vì thao tác tạo Auth user vốn đã cần service role, implementation đặt **manual membership grant sau authenticated Edge Function** thay vì thêm một RPC/schema delta. Business outcome, role guard, idempotency, audit và trusted-backend boundary vẫn được giữ.
- docs/worklog/2026-08-24/002-worklog-admin-workspace-simplification.md :: - `source = manual` đúng canonical membership constraint;
- docs/worklog/2026-08-24/002-worklog-admin-workspace-simplification.md :: Các file `AdminWorkspacePage`/Dashboard/Reconciliation/Reports/Config cũ không còn reachable từ `adminRouter`, nhưng chưa xóa vật lý trong đợt này. Lý do: plan chỉ cho xóa sau regression/runtime tests; Flutter runtime hiện không khả dụng. Giữ file source-only giúp rollback an toàn và không làm tăng rủi ro compile do delete rộng trước khi full test.
- docs/worklog/2026-08-24/002-worklog-admin-workspace-simplification.md :: 4. deploy sandbox, smoke create account/manual grant/payment/Sale payout.
...
- docs/worklog/2026-08-24/003-worklog-redundant-file-cleanup.md :: - `python3 tools/validate_docs_source_truth.py`: BASELINE BLOCKED - chỉ thiếu `docs/audit/source_truth_manifest.json`, không phát sinh lỗi cleanup mới.
- docs/worklog/2026-08-24/003-worklog-redundant-file-cleanup.md :: - `powershell -ExecutionPolicy Bypass -File .codex/tools/validate_codex_integrity.ps1`: SKIPPED - PowerShell không có trong môi trường hiện tại.
- docs/worklog/2026-08-24/003-worklog-redundant-file-cleanup.md :: ## Loi/Rui ro
- docs/worklog/2026-08-24/003-worklog-redundant-file-cleanup.md :: - Chua fix: source-truth manifest audit đang thiếu từ baseline; không thuộc phạm vi xóa file.
...
- docs/worklog/2026-09-13/003-worklog-admin-purge-gmail.md :: `Deno.serve`, khiến request timeout dù Function báo `ACTIVE`.
