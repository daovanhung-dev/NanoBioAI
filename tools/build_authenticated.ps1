...
function Invoke-FlutterBuild {
  ...
  if ($null -eq $exitCode) {
    throw "flutter build did not return an exit code."
  }
  if ($exitCode -ne 0) {
    throw "flutter build failed with exit code $exitCode."
  }
}

if ($Target -eq "appbundle" -and $Mode -ne "release") {
  throw "Android App Bundle should be built in release mode."
}
...
Write-Host "Building $Target ($Mode) with account access enabled."
