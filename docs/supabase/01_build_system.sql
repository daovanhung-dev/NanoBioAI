-- =============================================================================
-- 01_build_system.sql
-- NanoBio canonical local/sandbox system rebuild.
--
-- DESTRUCTIVE LOCAL/SANDBOX SCRIPT ONLY. Do not run against staging or
-- production. Run this file before 02_seed_data.sql with ON_ERROR_STOP enabled.
--
-- Contains public schema, RLS, RPCs, Storage infrastructure, runtime grants,
-- M31 Sleep Safety rollout, and fail-fast system assertions. The individual
-- transaction boundaries are intentional and preserved from the source bundle.
-- =============================================================================
...
create table if not exists public.users (
    ...
    constraint users_completed_onboarding_has_time check (
        onboarding_status <> 'completed'
        or onboarding_completed_at is not null
    )
);

create unique index if not exists idx_users_phone_unique_not_null on public.users (phone)
where
    phone is not null;
...
create table if not exists public.health_subjects (
    ...
    subject_type text not null default 'self' check (
        subject_type in ('self', 'family_member')
    ),
    display_name text,
    relationship text,
    gender text,
    ...
    created_at timestamptz not null default now(),
    updated_at timestamptz not null default now(),
    constraint health_subject_self_owner check (
        subject_type <> 'self'
        or owner_user_id = coalesce(linked_user_id, owner_user_id)
    )
);

create unique index if not exists idx_health_subjects_one_self_per_owner on public.health_subjects (owner_user_id)
where
    subject_type = 'self';

create index if not exists idx_health_subjects_owner_active on public.health_subjects (owner_user_id, is_active);

create index if not exists idx_health_subjects_linked_user on public.health_subjects (linked_user_id)
where
...
create policy health_subjects_select_allowed on public.health_subjects for
select to authenticated using (
        public.can_read_health_subject (id)
    );

create policy health_subjects_insert_own on public.health_subjects for
insert
    to authenticated
with
    check (
...
create policy health_subjects_update_allowed on public.health_subjects for
update to authenticated using (
    public.can_write_health_subject (id)
)
with
    check (
        public.can_write_health_subject (id)
...
create policy health_subjects_delete_own on public.health_subjects for delete to authenticated using (
    owner_user_id = (
        select auth.uid ()
    )
);
...
grant
select on public.users, public.health_subjects to authenticated;
...
,
update,
delete on public.health_subjects to authenticated;
...
create table if not exists public.personal_schedule_ai_requests (
    ...
    actor_mode text not null check (
        actor_mode in ('initial_guest', 'member_new')
    ),
...
create index if not exists idx_meal_plans_subject_date_order on public.meal_plans (
    subject_id,
    plan_date,
    meal_order
);

create index if not exists idx_daily_tasks_subject_date_order on public.daily_health_tasks (
    subject_id,
    task_date,
    sort_order
);

create index if not exists idx_lifestyle_schedule_subject_date_order on public.lifestyle_schedule_items (
    subject_id,
    schedule_date,
    sort_order
);
...
create index if not exists idx_health_score_ledgers_subject_period on public.health_score_ledgers (
    subject_id,
    period_end desc,
    formula_version
);

create index if not exists idx_wellness_point_ledgers_subject_date on public.wellness_point_ledgers (
    subject_id,
    schedule_date desc,
    program_code
);

create index if not exists idx_wellness_point_ledgers_source on public.wellness_point_ledgers (source_type, source_id);
...
create index if not exists idx_ai_recommendations_subject_unread on public.ai_recommendations (
    subject_id,
    is_read,
    created_at desc
);

create index if not exists idx_personal_schedule_ai_requests_user_mode on public.personal_schedule_ai_requests (
    user_id,
    actor_mode,
    status,
    updated_at desc
...
revoke all on public.ai_content_reports from public, anon, authenticated;
...
create policy personal_schedule_ai_requests_select_own on public.personal_schedule_ai_requests for
select to authenticated using (
        user_id = (
            select auth.uid ()
        )
...
create unique index if not exists idx_membership_subscriptions_provider_id on public.membership_subscriptions (
    provider,
...
create table if not exists public.usage_quota_rules (
    ...
    created_at timestamptz not null default now(),
    updated_at timestamptz not null default now(),
    constraint usage_quota_rule_limit_valid check (
        (
            period_unit = 'none'
            and max_count is null
        )
        or (
            period_unit <> 'none'
...
create index if not exists idx_usage_events_user_feature_created on public.usage_events (
    user_id,
    feature_key,
    created_at desc
);
...
create policy membership_plans_read_authenticated on public.membership_plans for
select to authenticated using (is_active = true);

create policy plan_entitlements_read_authenticated on public.plan_entitlements for
select to authenticated using (is_active = true);

create policy usage_quota_rules_read_authenticated on public.usage_quota_rules for
select to authenticated using (is_active = true);

drop policy if exists membership_subscriptions_select_own on public.membership_subscriptions;

create policy membership_subscriptions_select_own on public.membership_subscriptions for
select to authenticated using (
        user_id = (
            select auth.uid ()
        )
...
create policy usage_events_select_own on public.usage_events for
select to authenticated using (
        user_id = (
            select auth.uid ()
        )
...
revoke all on function public.check_usage_quota(uuid, text, text, text, timestamptz)
  from public, anon;

revoke all on function public.commit_usage_quota(
  uuid,
  text,
  text,
  text,
...
revoke all on function public.check_personal_schedule_generation_quota(
  uuid,
  text,
  text,
  text,
...
revoke all on function public.commit_personal_schedule_generation_quota(
  uuid,
  text,
  text,
  text,
...
grant
execute on function public.commit_usage_quota (
    uuid,
    text,
    text,
    text,
...
grant
execute on function public.check_personal_schedule_generation_quota (
    uuid,
    text,
    text,
    text,
...
grant
execute on function public.commit_personal_schedule_generation_quota (
    uuid,
    text,
    text,
    text,
...
create unique index if not exists idx_family_members_group_user_unique on public.family_members (family_group_id, user_id)
where
    user_id is not null
    and status <> 'removed';

create unique index if not exists idx_family_groups_owner_active_unique on public.family_groups (owner_user_id)
where
    status = 'active';

create index if not exists idx_family_groups_owner_status on public.family_groups (owner_user_id, status);

create index if not exists idx_family_members_subject on public.family_members (subject_id);
...
create policy family_groups_select_allowed on public.family_groups for
select to authenticated using (
        owner_user_id = (
            select auth.uid ()
        )
...
create policy family_members_select_allowed on public.family_members for
select to authenticated using (
        exists (
            select 1
            from public.family_groups fg
            where
                fg.id = family_group_id
                and fg.owner_user_id = (
                    select auth.uid ()
                )
        )
        or exists (
            ...
            where
                actor.family_group_id = family_members.family_group_id
                and actor.user_id = (
                    select auth.uid ()
                )
...
revoke all on function public.familyplus_context_for_user (uuid)
from public, anon, authenticated;

revoke all on function public.get_my_familyplus_context ()
from public, anon;

revoke all on function public.upsert_my_familyplus_group (text, text)
from public, anon;

revoke all on function public.upsert_my_familyplus_member (
    uuid,
    text,
    text,
    boolean,
...
revoke all on function public.remove_my_familyplus_member (uuid, text)
from public, anon;

grant
execute on function public.get_my_familyplus_context () to authenticated;

grant
execute on function public.upsert_my_familyplus_group (text, text) to authenticated;

grant
execute on function public.upsert_my_familyplus_member (
    uuid,
    text,
    text,
    boolean,
...
grant
execute on function public.remove_my_familyplus_member (uuid, text) to authenticated;
...
create unique index if not exists idx_referral_codes_active_sale_user on public.referral_codes (sale_user_id)
where
    status = 'active';
...
create index if not exists idx_referral_relationships_referrer on public.referral_relationships (
    referrer_user_id,
    created_at desc
);

create index if not exists idx_referral_relationships_referred_status on public.referral_relationships (referred_user_id, status);

create index if not exists idx_referral_relationships_device_hash on public.referral_relationships (device_hash)
where
...
create index if not exists idx_payment_events_payer_paid on public.payment_events (payer_user_id, paid_at desc);

create index if not exists idx_payment_events_status_created on public.payment_events (status, created_at desc);

create index if not exists idx_payment_events_status_paid on public.payment_events (status, paid_at desc);

create unique index if not exists uq_payment_events_transfer_reference on public.payment_events (transfer_reference)
where
    transfer_reference is not null;

create index if not exists idx_sale_profiles_status_created on public.sale_profiles (status, created_at desc);

create table if not exists public.commission_rates (
    code text primary key default 'direct_referral',
    rate numeric(5, 4) not null check (
        rate >= 0
        and rate <= 1
    ),
    is_active boolean not null default true,
...
create index if not exists idx_commission_records_receiver_created on public.commission_records (
    receiver_user_id,
    created_at desc
);

create index if not exists idx_commission_records_receiver_status_available on public.commission_records (
    receiver_user_id,
    status,
    available_at,
    created_at desc
...
create index if not exists idx_commission_records_payment on public.commission_records (payment_event_id);
...
create policy sale_profiles_select_own on public.sale_profiles for
select to authenticated using (
        user_id = (
            select auth.uid ()
        )
...
create policy referral_codes_select_own on public.referral_codes for
select to authenticated using (
        sale_user_id = (
            select auth.uid ()
        )
...
create policy referral_relationships_select_related on public.referral_relationships for
select to authenticated using (
        referrer_user_id = (
            select auth.uid ()
        )
...
create policy payment_events_select_payer on public.payment_events for
select to authenticated using (
        payer_user_id = (
            select auth.uid ()
        )
...
create policy commission_rates_read_authenticated on public.commission_rates for
select to authenticated using (is_active = true);

drop policy if exists commission_records_select_receiver on public.commission_records;

create policy commission_records_select_receiver on public.commission_records for
select to authenticated using (
        receiver_user_id = (
            select auth.uid ()
        )
...
grant
select on public.sale_profiles, public.referral_codes, public.referral_relationships, public.payment_events, public.commission_rates, public.commission_records to authenticated;

revoke
insert
,
update,
delete on public.sale_profiles,
...
add column if not exists admin_status text not null default 'active' check (
    admin_status in (
        'active',
        'suspended',
        'closed'
...
add column if not exists app_access_mode text not null default 'user' check (
    app_access_mode in ('user', 'admin', 'both')
);
...
create index if not exists idx_admin_user_roles_user_active on public.admin_user_roles (
    user_id,
    is_active,
    revoked_at
);

create index if not exists idx_system_config_versions_key_status_created on public.system_config_versions (
    config_key,
    status,
    created_at desc
);

create index if not exists idx_report_exports_requested_status_created on public.report_exports (
    requested_by,
    status,
    created_at desc
);

create index if not exists idx_sale_point_adjustments_user_status_created on public.sale_point_adjustments (
    sale_user_id,
    status,
    created_at desc
);

create index if not exists idx_admin_reconciliation_discrepancies_status_created on public.admin_reconciliation_discrepancies (status, created_at desc);

create index if not exists idx_admin_reconciliation_discrepancies_target on public.admin_reconciliation_discrepancies (target_id);

create index if not exists idx_admin_audit_events_created on public.admin_audit_events (created_at desc);

create index if not exists idx_admin_audit_events_target on public.admin_audit_events (target_id, created_at desc);
...
create policy admin_roles_read_admin on public.admin_roles for
select to authenticated using (
        public.admin_has_permission ('audit.read')
    );

drop policy if exists admin_permissions_read_admin on public.admin_permissions;

create policy admin_permissions_read_admin on public.admin_permissions for
select to authenticated using (
        public.admin_has_permission ('audit.read')
    );
...
create policy admin_role_permissions_read_admin on public.admin_role_permissions for
select to authenticated using (
        public.admin_has_permission ('audit.read')
    );
...
create policy admin_user_roles_read_self_or_admin on public.admin_user_roles for
select to authenticated using (
        user_id = (
            select auth.uid ()
        )
...
create policy admin_audit_events_read_admin on public.admin_audit_events for
select to authenticated using (
        public.admin_has_permission ('audit.read')
    );
...
create policy system_config_versions_read_admin on public.system_config_versions for
select to authenticated using (
...
create policy report_exports_read_admin on public.report_exports for
select to authenticated using (
        public.admin_has_permission ('reports.write')
    );

drop policy if exists sale_point_adjustments_read_admin on public.sale_point_adjustments;

create policy sale_point_adjustments_read_admin on public.sale_point_adjustments for
select to authenticated using (
        public.admin_has_permission ('points.write')
    );

drop policy if exists admin_reconciliation_runs_read_admin on public.admin_reconciliation_runs;

create policy admin_reconciliation_runs_read_admin on public.admin_reconciliation_runs for
select to authenticated using (
        public.admin_has_permission ('reconciliation.write')
    );

drop policy if exists admin_reconciliation_discrepancies_read_admin on public.admin_reconciliation_discrepancies;

create policy admin_reconciliation_discrepancies_read_admin on public.admin_reconciliation_discrepancies for
select to authenticated using (
        public.admin_has_permission ('reconciliation.write')
    );
...
select
    ...
    public.admin_reconciliation_discrepancies to authenticated;

revoke
insert
,
update,
delete on public.admin_roles,
public.admin_permissions,
public.admin_role_permissions,
public.admin_user_roles,
public.admin_audit_events,
...
grant
execute on function public.get_my_admin_session () to authenticated;

grant
execute on function public.get_admin_dashboard_summary (
    timestamptz,
    timestamptz,
    text,
    text
...
grant
execute on function public.create_membership_payment_request (
    public.nb_membership_plan,
    text,
    text,
    text
...
grant
execute on function public.confirm_my_membership_payment_transfer (uuid) to authenticated;

grant
execute on function public.get_my_membership_payment_request () to authenticated;

grant
execute on function public.admin_get_payment_review_alert () to authenticated;
...
grant
execute on function public.admin_list_payments (text, integer) to authenticated;

grant
execute on function public.admin_review_payment (uuid, text, text, text) to authenticated;

grant
execute on function public.admin_refund_or_cancel_payment (uuid, text, text, text) to authenticated;

grant
execute on function public.admin_list_sales (text, integer) to authenticated;

grant
execute on function public.admin_review_sale_profile (uuid, text, text, text) to authenticated;

grant
execute on function public.admin_upsert_config_version (text, jsonb, text, text) to authenticated;

grant
execute on function public.admin_list_config_versions (text, integer) to authenticated;

grant
execute on function public.admin_list_plan_config_versions (text, integer) to authenticated;

grant
execute on function public.admin_list_report_catalog (text, integer) to authenticated;

grant
execute on function public.admin_request_report_export (text, jsonb, text, text) to authenticated;

grant
execute on function public.admin_list_report_exports (text, integer) to authenticated;

grant
execute on function public.admin_adjust_sale_points (uuid, integer, text, text) to authenticated;

grant
execute on function public.admin_create_reconciliation_run (text, text, text) to authenticated;

grant
execute on function public.admin_list_reconciliation_discrepancies (text, integer) to authenticated;

grant
execute on function public.admin_update_reconciliation_discrepancy_status (uuid, text, text, text) to authenticated;

grant
execute on function public.admin_list_audit_events (text, integer) to authenticated;
...
revoke all on function public.create_sale_point_reversal_for_payment (uuid, text, text, text)
from public, anon, authenticated;

-- ---------------------------------------------------------------------------
-- 13b. Membership payment hardening final overlay
-- ---------------------------------------------------------------------------
-- Kept here after the Admin base functions so the system build finishes with
-- the same final M13 contract as the former numbered local/sandbox bundle.
...
alter table public.payment_events
add constraint payment_events_transfer_reference_format_check check (
    transfer_reference is null
    or transfer_reference ~ '^NB[0-9A-F]{12}$'
);
...
alter table public.payment_events
add constraint payment_events_transfer_memo_format_check check (
...
,
update,
delete on public.payment_events
from anon, authenticated;
...
revoke all on function public.admin_review_payment (
    uuid,
    text,
    text,
    text,
...
grant
execute on function public.cancel_my_membership_payment_request (uuid) to authenticated;
...
grant
execute on function public.admin_review_payment (
    uuid,
    text,
    text,
    text,
...
create unique index if not exists idx_sale_point_conversions_idempotency on public.sale_point_conversions (sale_user_id, idempotency_key)
where
    idempotency_key is not null;

create index if not exists idx_sale_point_conversions_sale_created on public.sale_point_conversions (sale_user_id, created_at desc);

create index if not exists idx_sale_point_conversions_status_created on public.sale_point_conversions (status, created_at desc);

create index if not exists idx_sale_point_conversions_sale_status_created on public.sale_point_conversions (
    sale_user_id,
    status,
    created_at desc
);
...
drop trigger if exists trg_sale_payout_profiles_updated_at on public.sale_payout_profiles;

create trigger trg_sale_payout_profiles_updated_at
  before update on public.sale_payout_profiles
  for each row execute function public.set_updated_at();

drop trigger if exists trg_sale_point_conversions_updated_at on public.sale_point_conversions;
...
drop policy if exists sale_point_conversions_select_own on public.sale_point_conversions;

create policy sale_point_conversions_select_own on public.sale_point_conversions for
select to authenticated using (
        sale_user_id = (
            select auth.uid ()
        )
...
drop policy if exists sale_payout_profiles_select_own on public.sale_payout_profiles;

create policy sale_payout_profiles_select_own on public.sale_payout_profiles for
select to authenticated using (
        sale_user_id = (
            select auth.uid ()
        )
...
,
update,
delete on public.sale_point_conversions
from anon, authenticated;

revoke all on
table public.sale_payout_profiles
from anon, authenticated;
...
revoke all on function public.get_my_sale_dashboard ()
from public, anon;

revoke all on function public.get_my_sale_direct_customers ()
from public, anon;

revoke all on function public.get_my_sale_point_ledger ()
from public, anon;

revoke all on function public.get_my_sale_conversions ()
from public, anon;
...
grant
execute on function public.get_my_sale_state () to authenticated;

grant
execute on function public.request_sale_participation (text, text) to authenticated;

grant
execute on function public.attach_my_referral_code (text, text) to authenticated;

grant
execute on function public.get_my_sale_payout_profile () to authenticated;

grant
execute on function public.upsert_my_sale_payout_profile (text, text, text, text, text) to authenticated;

grant
execute on function public.get_my_sale_dashboard () to authenticated;

grant
execute on function public.get_my_sale_direct_customers () to authenticated;

grant
execute on function public.get_my_sale_point_ledger () to authenticated;

grant
execute on function public.get_my_sale_conversions () to authenticated;

grant
execute on function public.request_sale_point_conversion (integer, text) to authenticated;

grant
execute on function public.admin_list_sale_point_conversions (text, integer) to authenticated;

grant
execute on function public.admin_review_sale_point_conversion (uuid, text, text, text, text) to authenticated;
...
-- existing tables/data. Apply to sandbox first. Do not execute this destructive
-- local/sandbox build on remote production.
...
create table if not exists public.schedule_completion_attempts (
    ...
    status text not null default 'begun' check (
        status in (
            'begun',
            'finalized',
            'undone',
...
create unique index if not exists idx_schedule_completion_proofs_one_active on public.schedule_completion_proofs (eligibility_id)
where
    status = 'active';
...
create table if not exists public.wellness_point_allocations (
    ...
    remaining_points integer not null check (
        remaining_points >= 0
        and remaining_points <= original_points
    ),
    status text not null check (
        status in (
            'pending',
...
create index if not exists idx_schedule_reward_eligibilities_user_window on public.schedule_reward_eligibilities (
    user_id,
    window_start,
    window_end
);

create index if not exists idx_schedule_completion_attempts_user_status on public.schedule_completion_attempts (
    user_id,
    status,
    began_at desc
);

create index if not exists idx_schedule_completion_proofs_user_created on public.schedule_completion_proofs (user_id, created_at desc);

create index if not exists idx_wellness_point_allocations_wallet_expiry on public.wellness_point_allocations (
    user_id,
    status,
    available_at,
    expires_at
...
create unique index if not exists idx_wellness_point_allocations_one_schedule_reward on public.wellness_point_allocations (eligibility_id)
where
    eligibility_id is not null;

create index if not exists idx_wellness_point_ledgers_reward_history on public.wellness_point_ledgers (
    user_id,
    is_redeemable,
    created_at desc
);

create index if not exists idx_wellness_reward_offers_catalog on public.wellness_reward_offers (
    is_active,
    available_from,
    available_until
);

create index if not exists idx_wellness_reward_codes_stock on public.wellness_reward_codes (offer_id, status, created_at);

create unique index if not exists idx_wellness_reward_codes_global_hash on public.wellness_reward_codes (code_hash);

create index if not exists idx_wellness_reward_redemptions_user_created on public.wellness_reward_redemptions (user_id, created_at desc);
...
drop trigger if exists trg_guest_schedule_reward_registrations_updated_at on public.guest_schedule_reward_registrations;
...
drop trigger if exists trg_member_schedule_reward_registrations_updated_at on public.member_schedule_reward_registrations;
...
drop trigger if exists trg_schedule_completion_attempts_updated_at on public.schedule_completion_attempts;

create trigger trg_schedule_completion_attempts_updated_at
  before update on public.schedule_completion_attempts
  for each row execute function public.set_updated_at();

drop trigger if exists trg_schedule_completion_proofs_updated_at on public.schedule_completion_proofs;

create trigger trg_schedule_completion_proofs_updated_at
  before update on public.schedule_completion_proofs
  for each row execute function public.set_updated_at();

drop trigger if exists trg_wellness_reward_wallets_updated_at on public.wellness_reward_wallets;

create trigger trg_wellness_reward_wallets_updated_at
  before update on public.wellness_reward_wallets
  for each row execute function public.set_updated_at();

drop trigger if exists trg_wellness_point_allocations_updated_at on public.wellness_point_allocations;

create trigger trg_wellness_point_allocations_updated_at
  before update on public.wellness_point_allocations
  for each row execute function public.set_updated_at();

drop trigger if exists trg_wellness_reward_offers_updated_at on public.wellness_reward_offers;

create trigger trg_wellness_reward_offers_updated_at
  before update on public.wellness_reward_offers
  for each row execute function public.set_updated_at();

drop trigger if exists trg_wellness_reward_redemptions_updated_at on public.wellness_reward_redemptions;
...
drop trigger if exists trg_wellness_point_ledgers_updated_at on public.wellness_point_ledgers;
...
drop trigger if exists trg_wellness_point_ledgers_append_only on public.wellness_point_ledgers;
...
drop policy if exists wellness_point_ledgers_select_subject on public.wellness_point_ledgers;

drop policy if exists wellness_point_ledgers_insert_subject on public.wellness_point_ledgers;

drop policy if exists wellness_point_ledgers_update_subject on public.wellness_point_ledgers;

drop policy if exists wellness_point_ledgers_delete_subject on public.wellness_point_ledgers;

drop policy if exists wellness_point_ledgers_select_own on public.wellness_point_ledgers;

create policy wellness_point_ledgers_select_own on public.wellness_point_ledgers for
select to authenticated using (
        user_id = (
            select auth.uid ()
        )
...
drop policy if exists schedule_reward_eligibilities_select_own on public.schedule_reward_eligibilities;

create policy schedule_reward_eligibilities_select_own on public.schedule_reward_eligibilities for
select to authenticated using (
        user_id = (
            select auth.uid ()
        )
...
drop policy if exists schedule_completion_attempts_select_own on public.schedule_completion_attempts;

create policy schedule_completion_attempts_select_own on public.schedule_completion_attempts for
select to authenticated using (
        user_id = (
            select auth.uid ()
        )
...
drop policy if exists schedule_completion_proofs_select_own on public.schedule_completion_proofs;

create policy schedule_completion_proofs_select_own on public.schedule_completion_proofs for
select to authenticated using (
        user_id = (
            select auth.uid ()
        )
...
drop policy if exists wellness_reward_wallets_select_own on public.wellness_reward_wallets;

create policy wellness_reward_wallets_select_own on public.wellness_reward_wallets for
select to authenticated using (
        user_id = (
            select auth.uid ()
        )
...
drop policy if exists wellness_point_allocations_select_own on public.wellness_point_allocations;

create policy wellness_point_allocations_select_own on public.wellness_point_allocations for
select to authenticated using (
        user_id = (
            select auth.uid ()
        )
...
drop policy if exists wellness_reward_offers_select_active on public.wellness_reward_offers;

create policy wellness_reward_offers_select_active on public.wellness_reward_offers for
select to authenticated using (is_active = true);

drop policy if exists wellness_reward_redemptions_select_own on public.wellness_reward_redemptions;

create policy wellness_reward_redemptions_select_own on public.wellness_reward_redemptions for
select to authenticated using (
        user_id = (
            select auth.uid ()
        )
...
revoke all on public.guest_schedule_reward_registrations,
...
,
update,
delete on public.wellness_point_ledgers
from anon, authenticated;

grant
select on public.schedule_reward_eligibilities, public.schedule_completion_attempts, public.schedule_completion_proofs, public.wellness_reward_wallets, public.wellness_point_ledgers, public.wellness_point_allocations, public.wellness_reward_offers, public.wellness_reward_redemptions to authenticated;
...
grant
execute on function public.can_access_schedule_proof_object (text) to authenticated;

drop policy if exists schedule_completion_proofs_storage_select_own on storage.objects;

create policy schedule_completion_proofs_storage_select_own on storage.objects for
select to authenticated using (
        bucket_id = 'schedule-completion-proofs'
        and public.can_access_schedule_proof_object (name)
    );

drop policy if exists schedule_completion_proofs_storage_insert_own on storage.objects;
...
drop policy if exists schedule_completion_proofs_storage_update_own on storage.objects;

drop policy if exists schedule_completion_proofs_storage_delete_own on storage.objects;
...
revoke all on function public.admin_cancel_reward_redemption (uuid, text, boolean, text)
from public, anon;

grant
execute on function public.register_my_schedule_reward_eligibilities (text, jsonb, text) to authenticated;

grant
execute on function public.begin_my_schedule_completion (uuid, text) to authenticated;

grant
execute on function public.finalize_my_schedule_completion (uuid, text, text) to authenticated;

grant
execute on function public.undo_my_schedule_completion (uuid, text) to authenticated;

grant
execute on function public.get_my_wellness_reward_summary () to authenticated;

grant
execute on function public.list_my_wellness_point_history (integer) to authenticated;

grant
execute on function public.list_my_reward_offers (integer) to authenticated;

grant
execute on function public.redeem_my_reward_offer (uuid, text) to authenticated;

grant
execute on function public.list_my_reward_redemptions (integer) to authenticated;

grant
execute on function public.get_my_reward_code (uuid) to authenticated;

grant
execute on function public.admin_list_wellness_rewards (text, integer) to authenticated;
...
grant
execute on function public.admin_cancel_reward_redemption (uuid, text, boolean, text) to authenticated;
...
create unique index if not exists idx_nabi_notification_one_active on public.nabi_notification_definitions (notification_id)
where
    status = 'active';
...
create index if not exists idx_nabi_notification_state_user_status on public.nabi_notification_user_states (
    user_id,
    status,
    eligible_at desc
);
...
create table if not exists public.nabi_notification_preferences (
    ...
    quiet_start_minutes integer check (
        quiet_start_minutes between 0 and 1439
    ),
    quiet_end_minutes integer check (
        quiet_end_minutes between 0 and 1439
    ),
    updated_at timestamptz not null default now()
);
...
create index if not exists idx_nabi_notification_events_user_created on public.nabi_notification_events (user_id, created_at desc);

create index if not exists idx_nabi_notification_events_retention on public.nabi_notification_events (created_at);
...
drop policy if exists nabi_notification_definitions_read_active on public.nabi_notification_definitions;

create policy nabi_notification_definitions_read_active on public.nabi_notification_definitions for
select to authenticated using (
        status = 'active'
        and (
...
drop policy if exists nabi_notification_state_read_own on public.nabi_notification_user_states;

create policy nabi_notification_state_read_own on public.nabi_notification_user_states for
select to authenticated using (user_id = auth.uid ());

drop policy if exists nabi_notification_preferences_read_own on public.nabi_notification_preferences;

create policy nabi_notification_preferences_read_own on public.nabi_notification_preferences for
select to authenticated using (user_id = auth.uid ());

drop policy if exists nabi_notification_events_read_own on public.nabi_notification_events;

create policy nabi_notification_events_read_own on public.nabi_notification_events for
select to authenticated using (user_id = auth.uid ());
...
revoke all on
table public.nabi_notification_definitions
from anon, authenticated;

revoke all on
table public.nabi_notification_user_states
from anon, authenticated;

revoke all on
table public.nabi_notification_preferences
from anon, authenticated;

revoke all on
table public.nabi_notification_events
from anon, authenticated;

grant
select on public.nabi_notification_definitions to authenticated;

grant
select on public.nabi_notification_user_states to authenticated;

grant
select on public.nabi_notification_preferences to authenticated;

grant select on public.nabi_notification_events to authenticated;

revoke all on function public.get_active_nabi_notification_definitions ()
from public, anon;

revoke all on function public.claim_nabi_notification_occurrence (
    text,
    integer,
    text,
    text,
...
revoke all on function public.upsert_my_nabi_notification_preferences (
    boolean,
    boolean,
    boolean,
    integer,
...
revoke all on function public.record_nabi_notification_events (jsonb)
from public, anon;
...
grant
execute on function public.get_active_nabi_notification_definitions () to authenticated;

grant
execute on function public.claim_nabi_notification_occurrence (
    text,
    integer,
    text,
    text,
...
grant
execute on function public.upsert_my_nabi_notification_preferences (
    boolean,
    boolean,
    boolean,
    integer,
...
grant
execute on function public.record_nabi_notification_events (jsonb) to authenticated;
...
create policy sale_payout_proofs_storage_select_admin on storage.objects for
select to authenticated using (
...
create policy sale_payout_proofs_storage_insert_admin on storage.objects for
insert
    to authenticated
with
    check (
...
create index if not exists idx_meal_catalog_topic_active on public.meal_catalog (health_topic_code, is_active);

create index if not exists idx_meal_catalog_plan_pool on public.meal_catalog (
    meal_type,
    is_active,
    is_plan_eligible,
    metadata_status
...
create index if not exists idx_meal_catalog_source_version on public.meal_catalog (source_hash, version);
...
create table if not exists public.medication_records (
    ...
    usage_schedule text,
    prescriber_confirmed boolean not null default false,
    note text,
    is_active boolean not null default true,
    created_at timestamptz not null default now(),
    updated_at timestamptz not null default now()
...
create policy meal_catalog_read_active_safe on public.meal_catalog for
...
-- =============================================================================
-- END MEAL NUTRITION V18
-- =============================================================================

-- =============================================================================
-- BEGIN DAILY HEALTH HUB REWARDS
-- =============================================================================
...
-- ---------------------------------------------------------------------------
-- The base system schema intentionally treats
-- schedule_reward_eligibilities + active JPEG
...
-- snapshots from the base system schema; only completion state
  -- is changed here.
  with latest as (
...
-- =============================================================================
-- END DAILY HEALTH HUB REWARDS
-- =============================================================================

-- =============================================================================
-- BEGIN AUTH ACCOUNT LOCK
-- =============================================================================
...
-- =============================================================================
-- END AUTH ACCOUNT LOCK
-- =============================================================================

-- =============================================================================
-- BEGIN SLEEP SAFETY M31 SCHEMA
-- =============================================================================
...
-- =============================================================================
-- END SLEEP SAFETY M31 SCHEMA
-- =============================================================================

-- =============================================================================
-- BEGIN RUNTIME STORAGE AND RPC SUPPORT
-- =============================================================================
...
-- The system sections above create these RPCs. This manifest keeps the
-- app-facing API explicit and fails the rebuild before a missing function
...
-- =============================================================================
-- END RUNTIME STORAGE AND RPC SUPPORT
-- =============================================================================

-- =============================================================================
-- BEGIN SLEEP SAFETY M31 ROLLOUT
-- =============================================================================
...
-- Daily Health Hub static contract.
do $$
declare
  v_missing_functions text[];
begin
  if to_regclass('public.schedule_health_checkins') is null then
    raise exception 'DAILY_HEALTH_HUB_TABLE_MISSING';
  end if;

  if public.schedule_health_action_reward_points('hydration') <> 4
     or public.schedule_health_action_reward_points('mood_stress') <> 5
     or public.schedule_health_action_reward_points('sleep_checkin') <> 6
     or public.schedule_health_action_reward_points('weight_checkin') <> 4
     or public.schedule_health_action_reward_points('quick_complete') <> 5 then
    raise exception 'DAILY_HEALTH_HUB_REWARD_POINTS_INVALID';
  end if;
  ...
  if v_missing_functions is not null then
    raise exception 'DAILY_HEALTH_HUB_RPC_MISSING_%', array_to_string(v_missing_functions, ',');
  end if;

  if not has_table_privilege('authenticated', 'public.schedule_health_checkins', 'SELECT')
     or has_table_privilege('authenticated', 'public.schedule_health_checkins', 'INSERT')
     or has_table_privilege('authenticated', 'public.schedule_health_checkins', 'UPDATE')
     or has_table_privilege('authenticated', 'public.schedule_health_checkins', 'DELETE') then
    raise exception 'DAILY_HEALTH_HUB_GRANT_INVALID';
  end if;
  ...
  if not exists (
    ...
    raise exception 'DAILY_HEALTH_HUB_ELIGIBILITY_NULLABILITY_INVALID';
  end if;
...
-- AI runtime database contract. Provider deployment and Edge secrets are
-- deliberately outside PostgreSQL and validated separately.
do $$
declare
...
begin
  ...
  raise notice 'PASS AI runtime database contract';
end
$$;

-- Runtime infrastructure smoke runs in a rollback-only transaction so it
-- cannot leave state behind in the rebuilt sandbox.
begin;
...
begin
  ...
  if not exists (
    select 1
    from public.sleep_safety_runtime_config
    where config_key = 'default'
      and enabled = true
  ) then
    raise exception 'SLEEP_SAFETY_ROLLOUT_NOT_ENABLED';
  end if;
  ...
  if not has_function_privilege(
    'authenticated',
    'public.upsert_sleep_safety_contact(uuid,text,text,text,integer)'::regprocedure,
    'EXECUTE'
  ) then
  ...
  if not has_function_privilege(
    'authenticated',
    'public.delete_sleep_safety_contact(uuid)'::regprocedure,
    'EXECUTE'
  ) then
    raise exception 'RUNTIME_RPC_GRANT_MISSING_delete_sleep_safety_contact';
  end if;

  raise notice 'PASS runtime support: RPC, view, trigger, Storage, grants and M31 rollout available.';
end;
$$;
...
-- =============================================================================
-- END FAIL-FAST SYSTEM VALIDATION
-- =============================================================================
