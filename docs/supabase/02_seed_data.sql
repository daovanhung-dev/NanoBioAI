-- =============================================================================
-- 02_seed_data.sql
-- NanoBio canonical local/sandbox data seed.
--
-- DESTRUCTIVE LOCAL/SANDBOX SCRIPT ONLY. Do not run against staging or
-- production. Run only after 01_build_system.sql with ON_ERROR_STOP enabled.
--
-- Contains reference/config data, the canonical meal catalog, and all dev/test
-- accounts and fixtures. The initial transaction truncates Supabase Auth users.
-- =============================================================================
...
-- ---------------------------------------------------------------------------
-- Wipes Supabase Auth users/identities/sessions. The preceding schema rebuild
-- has already recreated every app object in public.
...
--   Thuytien8994@gmail.com   -> plus (local/sandbox only)
...
select
  ...
  crypt(
    case
      when email = 'Thuytien8994@gmail.com' then '1234567890'
      else 'NanoBio@123456'
    end,
    gen_salt('bf')
  ),
...
do $$
declare
  v_user_id constant uuid := '10000000-0000-4000-8000-000000000105'::uuid;
begin
  if not exists (
    select 1
    from auth.users au
    where au.id = v_user_id
      and au.email = 'Thuytien8994@gmail.com'
      and au.email_confirmed_at is not null
      and crypt('1234567890', au.encrypted_password) = au.encrypted_password
  ) then
    raise exception 'LOCAL_PLUS_SEED_AUTH_INVALID';
  end if;

  if public.current_plan_for_user(v_user_id) <> 'plus' then
    raise exception 'LOCAL_PLUS_SEED_PLAN_INVALID';
  end if;

  if not exists (
    select 1
    from public.plan_entitlements pe
    where pe.plan_code = 'plus'
      and pe.entitlement_key = 'ai_chat'
      and pe.is_active = true
      and pe.entitlement_value @> '{"enabled":true,"unlimited":true}'::jsonb
  ) then
    raise exception 'LOCAL_PLUS_SEED_AI_CHAT_ENTITLEMENT_INVALID';
  end if;
end
$$;
...
-- This fixture is embedded directly in the canonical seed transaction. It
-- deliberately has no nested transaction wrapper or module markers.
...
-- BEGIN GENERATED MEAL CATALOG

-- ---------------------------------------------------------------------------
-- Canonical meal catalog generated from docs/note/Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md
...
Nêm nếm lại cho vừa miệng.',0,0,0,0,0,0,'c01_thieu_mau','Thiếu máu','Thiếu máu là tình trạng cơ thể không sản xuất đủ hồng cầu hoặc hemoglobin, dẫn đến các triệu chứng như mệt mỏi, da xanh xao, hoa mắt, chóng mặt. Chế độ ăn giàu sắt và vitamin C có thể giúp cải thiện tình trạng này.',1,'Tim Mạch Và Mạch Máu Não','["200g thịt bò", "200g rau cải bó xôi", "1 củ hành tím", "Gia vị: muối, tiêu, dầu ăn"]'::jsonb,'["Thịt bò thái mỏng, ướp với chút muối và tiêu.", "Xào hành tím với dầu, cho thịt bò vào xào nhanh.", "Thêm nước, đun sôi rồi cho cải bó xôi vào nấu chín.", "Nêm nếm lại cho vừa miệng."]'::jsonb,'Thịt bò giàu sắt heme dễ hấp thụ, cải bó xôi giàu vitamin C giúp cơ thể hấp thụ sắt tốt hơn.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',6,'Chương 1: Tim Mạch Và Mạch Máu Não','Thiếu máu',1,'f973f9d388e032d768797ea217e8d83fa678aa1d21d43e84eb0ff5eae50ff44c',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Xay nhuyễn và thưởng thức.',0,0,0,0,0,0,'c01_thieu_mau','Thiếu máu','Thiếu máu là tình trạng cơ thể không sản xuất đủ hồng cầu hoặc hemoglobin, dẫn đến các triệu chứng như mệt mỏi, da xanh xao, hoa mắt, chóng mặt. Chế độ ăn giàu sắt và vitamin C có thể giúp cải thiện tình trạng này.',1,'Tim Mạch Và Mạch Máu Não','["1 quả chuối chín", "30g hạt điều", "200ml sữa hạnh nhân", "1 thìa mật ong"]'::jsonb,'["Chuối bóc vỏ, cắt nhỏ.", "Hạt điều ngâm nước 10 phút, sau đó cho vào máy xay cùng chuối, sữa hạnh nhân và mật ong.", "Xay nhuyễn và thưởng thức."]'::jsonb,'Tăng cường hấp thu sắt tự nhiên, Giảm mệt mỏi, chóng mặt, uể oải do thiếu máu gây ra, Hỗ trợ quá trình tạo máu.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',6,'Chương 1: Tim Mạch Và Mạch Máu Não','Thiếu máu',2,'9859286f5f880f3c86f9e08516c45141c6d92f66598a9ad575cbfad967218c25',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Nêm nếm lại cho vừa miệng.',0,0,0,0,0,0,'c01_benh_mach_vanh','Bệnh mạch vành','Bệnh mạch vành là tình trạng động mạch vành bị tắc nghẽn do mảng bám, dẫn đến giảm lưu thông máu đến tim. Chế độ ăn giàu chất xơ và chất chống oxy hóa có thể giúp cải thiện tình trạng này.',1,'Tim Mạch Và Mạch Máu Não','["200g nấm đông cô", "100g rau ngót", "1 củ hành tím", "Gia vị: muối, tiêu, dầu ăn"]'::jsonb,'["Nấm đông cô ngâm nở, rửa sạch.", "Rau ngót nhặt sạch, rửa kỹ.", "Phi hành tím với dầu, cho nấm vào xào sơ.", "Thêm nước, đun sôi rồi cho rau ngót vào nấu chín.", "Nêm nếm lại cho vừa miệng."]'::jsonb,'Nấm đông cô giàu chất chống oxy hóa giúp bảo vệ tim mạch, rau ngót hỗ trợ lưu thông máu và thanh lọc cơ thể',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',7,'Chương 1: Tim Mạch Và Mạch Máu Não','Bệnh mạch vành',1,'6e0b5ab892edb7a86d0a9bc9cfb716c5a448e12312572441740af50fee1fc4c1',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Nêm nếm lại cho vừa miệng.',0,0,0,0,0,0,'c01_roi_loan_nhip_tim','Rối loạn nhịp tim','Rối loạn nhịp tim là tình trạng nhịp tim không đều, có thể quá nhanh, quá chậm hoặc không đều. Chế độ ăn giàu kali và magie có thể giúp ổn định nhịp tim.',1,'Tim Mạch Và Mạch Máu Não','["200g đu đủ xanh", "300g sườn non", "1 củ hành tím", "Gia vị: muối, tiêu, dầu ăn"]'::jsonb,'["Sườn non rửa sạch, chần qua nước sôi.", "Đu đủ gọt vỏ, bỏ hạt, cắt khúc.", "Phi hành tím với dầu, cho sườn vào xào sơ.", "Thêm nước, đun sôi rồi cho đu đủ vào hầm cho mềm", "Nêm nếm lại cho vừa miệng."]'::jsonb,'Đu đủ chứa kali hỗ trợ ổn định nhịp tim, sườn non cung cấp collagen giúp bảo vệ mạch máu..',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',8,'Chương 1: Tim Mạch Và Mạch Máu Não','Rối loạn nhịp tim',2,'9737b1a0e34c9a64bf73a4104cce82379be7fc6608638c74d3c5d85d7b145b14',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Uống ngay.',0,0,0,0,0,0,'c01_roi_loan_nhip_tim','Rối loạn nhịp tim','Rối loạn nhịp tim là tình trạng nhịp tim không đều, có thể quá nhanh, quá chậm hoặc không đều. Chế độ ăn giàu kali và magie có thể giúp ổn định nhịp tim.',1,'Tim Mạch Và Mạch Máu Não','["1 củ gừng nhỏ", "1 thìa mật ong", "200ml nước sôi"]'::jsonb,'["Gừng rửa sạch, thái lát.", "Cho gừng vào nước sôi, hãm trong 10 phút.", "Thêm mật ong, khuấy đều.", "Uống ngay."]'::jsonb,'Gừng có tính ấm, giúp lưu thông khí huyết, mật ong giúp tăng cường sức đề kháng, hỗ trợ ổn định nhịp tim.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',8,'Chương 1: Tim Mạch Và Mạch Máu Não','Rối loạn nhịp tim',3,'8b7c2a603e3e10ee59f6ddfff421fb5e4eb692fe050f4cfac71f8d1e8ef3311f',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Thêm đường phèn vào và khuấy đều.',0,0,0,0,0,0,'c01_huyet_ap_thap','Huyết áp thấp','Huyết áp thấp (hay còn gọi là hạ huyết áp) là tình trạng khi huyết áp trong động mạch giảm xuống dưới mức bình thường, gây ra các triệu chứng như chóng mặt, mệt mỏi, ngất xỉu, nhức đầu và thậm chí khó thở. Huyết áp thấp có thể do nhiều nguyên nhân, bao gồm mất nước, thiếu hụt dinh dưỡng, các bệnh lý về tim mạch hoặc rối loạn nội tiết.',1,'Tim Mạch Và Mạch Máu Não','["100g đậu đỏ", "50g hạt sen", "50g gạo nếp", "1 ít đường phèn"]'::jsonb,'["Đậu đỏ và hạt sen ngâm qua đêm cho mềm.", "Gạo nếp vo sạch, cho vào nồi nấu cùng đậu đỏ và hạt sen.", "Đun sôi, hạ nhỏ lửa và nấu cho đến khi cháo nhừ.", "Thêm đường phèn vào và khuấy đều."]'::jsonb,'Đậu đỏ giúp bổ sung sắt, cải thiện sức khỏe tuần hoàn, hạt sen giúp an thần và hỗ trợ sức khỏe tim mạch, làm ổn định huyết áp.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',11,'Chương 1: Tim Mạch Và Mạch Máu Não','Huyết áp thấp',3,'782fde8972c7649d4354b82126248e7a3f2319eb8eeadb669647a6167618f460',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Nêm gia vị vừa ăn.',0,0,0,0,0,0,'c02_chung_mat_ngu_va_mo_nhieu','Chứng mất ngủ và mơ nhiều','Mất ngủ là tình trạng khó ngủ hoặc thức giấc giữa đêm và không thể ngủ lại. Mơ nhiều có thể đi kèm với lo âu hoặc căng thẳng, rối loạn tâm lý, chế độ ăn thiếu vitamin B, magiê, melatonin.',2,'Bệnh Thần Kinh','["100g đậu phụ", "100g rau mồng tơi", "Gia vị: muối, tiêu, dầu ăn"]'::jsonb,'["Đậu phụ cắt miếng vừa ăn, rau mồng tơi rửa sạch.", "Phi hành tím với dầu ăn cho thơm, cho đậu phụ vào xào sơ.", "Thêm nước vào nồi, đun sôi rồi cho rau mồng tơi vào nấu chín.", "Nêm gia vị vừa ăn."]'::jsonb,'Đậu phụ giúp thư giãn cơ thể, rau mồng tơi có tác dụng an thần, giúp cải thiện giấc ngủ.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',12,'Chương 2: Bệnh Thần Kinh','Chứng mất ngủ và mơ nhiều',1,'3fe7adae3309a72bb6767a3ee67887d3c6f176dbb95a1361c036bddc4a1040b7',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Thêm mật ong vào và khuấy đều.',0,0,0,0,0,0,'c02_chung_mat_ngu_va_mo_nhieu','Chứng mất ngủ và mơ nhiều','Mất ngủ là tình trạng khó ngủ hoặc thức giấc giữa đêm và không thể ngủ lại. Mơ nhiều có thể đi kèm với lo âu hoặc căng thẳng, rối loạn tâm lý, chế độ ăn thiếu vitamin B, magiê, melatonin.',2,'Bệnh Thần Kinh','["1 quả cam", "1 quả chanh", "1 ít mật ong"]'::jsonb,'["Cam và chanh vắt lấy nước.", "Thêm mật ong vào và khuấy đều."]'::jsonb,'Cam và chanh giúp cung cấp vitamin C, làm dịu thần kinh, hỗ trợ giấc ngủ.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',13,'Chương 2: Bệnh Thần Kinh','Chứng mất ngủ và mơ nhiều',2,'ba2247972698eed36ca2c2bead7c8a5e0496eab8d48ee2a751407bd82b1ecb5b',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Xay nhuyễn và lọc qua rây.',0,0,0,0,0,0,'c02_chung_dau_dau_va_u_tai','Chứng đau đầu và ù tai','Đau đầu và ù tai có thể là dấu hiệu của căng thẳng, vấn đề về tuần hoàn máu, hoặc các vấn đề về thính giác. Nguyên nhân: Stress, rối loạn tuần hoàn máu, tắc nghẽn mạch máu, viêm tai, bệnh lý về thần kinh.',2,'Bệnh Thần Kinh','["200g dưa hấu", "10 lá bạc hà"]'::jsonb,'["Dưa hấu gọt vỏ, cắt miếng nhỏ.", "Lá bạc hà rửa sạch, cho vào máy xay với dưa hấu.", "Xay nhuyễn và lọc qua rây."]'::jsonb,'Dưa hấu giúp giải độc cơ thể và giảm viêm, bạc hà làm dịu thần kinh, giảm đau đầu và ù tai.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',14,'Chương 2: Bệnh Thần Kinh','Chứng đau đầu và ù tai',2,'4bf4ba9e18506476db1b107a5cf40cfdec18a79059f0d93c69cf96fc4409f848',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Nêm nếm lại cho vừa miệng.',0,0,0,0,0,0,'c03_viem_phoi','Viêm phổi','Viêm phổi là tình trạng viêm nhiễm ở phổi, thường do vi khuẩn, virus hoặc nấm gây ra. Chế độ ăn uống giàu chất dinh dưỡng và tăng cường hệ miễn dịch có thể giúp hỗ trợ điều trị.',3,'Hô Hấp','["1 con gà ác", "50g kỷ tử", "50g táo đỏ", "1 củ gừng", "Gia vị: muối, tiêu"]'::jsonb,'["Gà làm sạch, chặt miếng vừa ăn.", "Cho gà, kỷ tử, táo đỏ và gừng vào nồi hầm.", "Hầm đến khi gà chín mềm.", "Nêm nếm lại cho vừa miệng."]'::jsonb,'Gà hầm thuốc Bắc giúp bồi bổ cơ thể, tăng cường hệ miễn dịch, hỗ trợ điều trị viêm phổi.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',18,'Chương 3: Hô Hấp','Viêm phổi',1,'cfb44725d370d2493ef41a64eacfc9c718437dcd8179a1c84ecee2a5914104e4',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Đổ nước sôi vào, hãm trong 5-7 phút.
Uống ngay.',0,0,0,0,0,0,'c03_viem_phoi','Viêm phổi','Viêm phổi là tình trạng viêm nhiễm ở phổi, thường do vi khuẩn, virus hoặc nấm gây ra. Chế độ ăn uống giàu chất dinh dưỡng và tăng cường hệ miễn dịch có thể giúp hỗ trợ điều trị.',3,'Hô Hấp','["1 túi trà xanh", "1 nhánh bạc hà tươi", "200ml nước sôi"]'::jsonb,'["Cho túi trà xanh và bạc hà vào cốc.", "Đổ nước sôi vào, hãm trong 5-7 phút.", "Uống ngay."]'::jsonb,'Trà xanh và bạc hà có tính kháng viêm, giúp làm dịu cổ họng và hỗ trợ điều trị viêm phổi.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',19,'Chương 3: Hô Hấp','Viêm phổi',3,'e8c8634bd598566f37030f9728304f3b4c86004dc1c36a981d1d1328a32c2792',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Cho gừng vào nước sôi, hãm trong 10 phút.
Thêm mật ong, khuấy đều.
Uống ngay.',0,0,0,0,0,0,'c03_viem_phe_quan_man_tinh','Viêm phế quản mãn tính','Viêm phế quản mãn tính là tình trạng viêm nhiễm kéo dài ở đường hô hấp, gây ho và khó thở. Chế độ ăn uống giàu chất chống oxy hóa và kháng viêm có thể giúp cải thiện tình trạng này.',3,'Hô Hấp','["1 củ gừng nhỏ", "1 thìa mật ong", "200ml nước sôi"]'::jsonb,'["Gừng rửa sạch, thái lát.", "Cho gừng vào nước sôi, hãm trong 10 phút.", "Thêm mật ong, khuấy đều.", "Uống ngay."]'::jsonb,'Gừng có tính kháng viêm, mật ong giúp làm dịu cổ họng, hỗ trợ điều trị viêm phế quản mãn tính.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',20,'Chương 3: Hô Hấp','Viêm phế quản mãn tính',3,'6215a5fa35df50a428b4ecdbb2a27b3eeb56fc12740c5f7ab3998e7416405985',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Xay nhuyễn và dùng ngay.',0,0,0,0,0,0,'c04_viem_da_day','Viêm dạ dày','Viêm dạ dày là tình trạng viêm niêm mạc dạ dày, gây đau và khó chịu. Chế độ ăn uống nhẹ nhàng và giàu chất dinh dưỡng có thể giúp cải thiện tình trạng này.',4,'Tiêu Hóa','["1 quả bơ", "200ml sữa chua không đường", "1 thìa mật ong"]'::jsonb,'["Bơ bóc vỏ, bỏ hạt, cắt nhỏ.", "Cho bơ, sữa chua và mật ong vào máy xay sinh tố.", "Xay nhuyễn và dùng ngay."]'::jsonb,'Bơ và sữa chua giàu chất béo lành mạnh, giúp làm dịu niêm mạc dạ dày.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',21,'Chương 4: Tiêu Hóa','Viêm dạ dày',2,'863a2d78de4a791ad45a858485bf14896f6d9028b647151485721d091fdb47a3',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Nêm nếm lại cho vừa miệng.',0,0,0,0,0,0,'c04_viem_da_day','Viêm dạ dày','Viêm dạ dày là tình trạng viêm niêm mạc dạ dày, gây đau và khó chịu. Chế độ ăn uống nhẹ nhàng và giàu chất dinh dưỡng có thể giúp cải thiện tình trạng này.',4,'Tiêu Hóa','["300g bí đỏ", "200g thịt gà", "1 củ hành tím", "Gia vị: muối, tiêu, dầu ăn"]'::jsonb,'["Bí đỏ gọt vỏ, cắt miếng vừa ăn.", "Thịt gà rửa sạch, thái miếng.", "Phi hành tím với dầu, cho thịt gà vào xào sơ.", "Thêm nước, đun sôi rồi cho bí đỏ vào nấu chín.", "Nêm nếm lại cho vừa miệng."]'::jsonb,'Bí đỏ giàu chất xơ, thịt gà giàu protein, giúp làm dịu niêm mạc dạ dày.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',21,'Chương 4: Tiêu Hóa','Viêm dạ dày',3,'acf0b2d34d95ec86dc125454f079b6c7beea7736e42079fe089e1a78dc9c8d1d',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Nêm nếm lại cho vừa miệng.',0,0,0,0,0,0,'c04_benh_sa_da_day','Bệnh sa dạ dày','Bệnh sa dạ dày là tình trạng dạ dày bị sa xuống thấp hơn vị trí bình thường, gây khó tiêu và đau bụng. Chế độ ăn uống nhẹ nhàng và giàu chất dinh dưỡng có thể giúp cải thiện tình trạng này.',4,'Tiêu Hóa','["200g rau cần tây", "1 củ cà rốt", "1 củ hành tím", "Gia vị: muối, tiêu, dầu ăn"]'::jsonb,'["Rau cần tây rửa sạch, cắt khúc.", "Cà rốt gọt vỏ, thái lát.", "Phi hành tím với dầu, cho cà rốt vào xào sơ.", "Thêm nước, đun sôi rồi cho rau cần tây vào nấu chín.", "Nêm nếm lại cho vừa miệng."]'::jsonb,'Rau cần tây và cà rốt giàu chất xơ, giúp cải thiện tiêu hóa và hỗ trợ điều trị sa dạ dày.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',23,'Chương 4: Tiêu Hóa','Bệnh sa dạ dày',3,'48fdc36f422603e20128cb2a1a17a299c4dfa0d706d59713570e2452e6242fbc',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Phi hành tím với dầu, cho nấm vào xào sơ.
Thêm nước, đun sôi rồi cho đậu hũ vào nấu chín.
Nêm nếm lại cho vừa miệng.',0,0,0,0,0,0,'c04_ung_thu_da_day','Ung thư dạ dày','Ung thư dạ dày là tình trạng các tế bào ác tính phát triển trong dạ dày. Chế độ ăn uống giàu chất chống oxy hóa và tăng cường hệ miễn dịch có thể giúp hỗ trợ điều trị.',4,'Tiêu Hóa','["200g nấm hương", "200g đậu hũ", "1 củ hành tím", "Gia vị: muối, tiêu, dầu ăn"]'::jsonb,'["Nấm hương ngâm nở, rửa sạch.", "Đậu hũ cắt miếng vừa ăn.", "Phi hành tím với dầu, cho nấm vào xào sơ.", "Thêm nước, đun sôi rồi cho đậu hũ vào nấu chín.", "Nêm nếm lại cho vừa miệng."]'::jsonb,'Nấm hương có tính kháng viêm, đậu hũ giàu protein giúp tăng cường hệ miễn dịch.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',24,'Chương 4: Tiêu Hóa','Ung thư dạ dày',2,'c1e579e0c783d981ac433fc2e248c53b5f04964584dfe7ca076f501683cc400b',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Gừng thái lát mỏng.
Cho vào máy xay, xay nhuyễn và lọc qua rây.',0,0,0,0,0,0,'c04_viem_da_day_ruot_cap_tinh','Viêm dạ dày ruột cấp tính','Viêm dạ dày ruột cấp tính là tình trạng viêm niêm mạc dạ dày và ruột, thường do vi khuẩn, virus hoặc ngộ độc thực phẩm gây ra. Triệu chứng phổ biến bao gồm đau bụng, tiêu chảy, buồn nôn, và nôn mửa. Chế độ ăn uống nhẹ nhàng, dễ tiêu sẽ giúp hỗ trợ phục hồi nhanh chóng.',4,'Tiêu Hóa','["200g dưa hấu", "1 lát gừng tươi"]'::jsonb,'["Dưa hấu gọt vỏ, cắt miếng nhỏ.", "Gừng thái lát mỏng.", "Cho vào máy xay, xay nhuyễn và lọc qua rây."]'::jsonb,'Dưa hấu giúp giải độc cơ thể, trong khi gừng giúp giảm viêm và hỗ trợ tiêu hóa.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',26,'Chương 4: Tiêu Hóa','Viêm dạ dày ruột cấp tính',2,'932e4924fcda94cda365f158f45a230f9c300fbc441830e09693e41a40f6a8ab',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Cho vào máy xay sinh tố, xay nhuyễn.
Lọc qua rây và thưởng thức.',0,0,0,0,0,0,'c04_viem_ruot_man_tinh','Viêm ruột mãn tính','Viêm ruột mãn tính là tình trạng viêm kéo dài ở ruột, gây tiêu chảy mãn tính, đau bụng và mệt mỏi. Nguyên nhân có thể là bệnh Crohn, viêm đại tràng hoặc rối loạn miễn dịch.',4,'Tiêu Hóa','["2 củ cà rốt", "1 củ cải đỏ"]'::jsonb,'["Cà rốt và củ cải đỏ gọt vỏ, cắt miếng vừa ăn.", "Cho vào máy xay sinh tố, xay nhuyễn.", "Lọc qua rây và thưởng thức."]'::jsonb,'Cà rốt và củ cải đỏ cung cấp nhiều vitamin giúp tăng cường sức khỏe ruột, hỗ trợ phục hồi viêm ruột mãn tính.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',27,'Chương 4: Tiêu Hóa','Viêm ruột mãn tính',2,'0cd6425b93d1e9f6aff2597471f8de02def1396b42113f7bae17ca875b065336',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Nêm gia vị vừa ăn.',0,0,0,0,0,0,'c04_viem_ruot_man_tinh','Viêm ruột mãn tính','Viêm ruột mãn tính là tình trạng viêm kéo dài ở ruột, gây tiêu chảy mãn tính, đau bụng và mệt mỏi. Nguyên nhân có thể là bệnh Crohn, viêm đại tràng hoặc rối loạn miễn dịch.',4,'Tiêu Hóa','["200g đậu bắp", "100g nấm rơm", "1 củ hành tím", "Gia vị: muối, tiêu"]'::jsonb,'["Đậu bắp rửa sạch, cắt khúc. Nấm rơm rửa sạch, cắt đôi.", "Phi hành với dầu ăn cho thơm, cho nấm rơm vào xào sơ.", "Thêm nước, đun sôi rồi cho đậu bắp vào nấu chín.", "Nêm gia vị vừa ăn."]'::jsonb,'Đậu bắp giàu chất nhầy giúp bôi trơn ruột, giảm táo bón, nấm rơm hỗ trợ tiêu hóa.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',28,'Chương 4: Tiêu Hóa','Viêm ruột mãn tính',3,'65e5ddc5cc378c419c2ebdaf2c5ec71e80414972fdd79bdb3ca378938711db62',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Cho vào máy xay sinh tố, xay nhuyễn.
Lọc qua rây và uống ngay.',0,0,0,0,0,0,'c04_benh_tri','Bệnh trĩ','Bệnh trĩ là tình trạng tĩnh mạch ở hậu môn bị sưng, đau và có thể gây chảy máu. Nguyên nhân chủ yếu là táo bón mãn tính, mang vác nặng hoặc ngồi lâu.',4,'Tiêu Hóa','["1 củ cà rốt", "1 củ dền"]'::jsonb,'["Cà rốt và củ dền gọt vỏ, cắt miếng nhỏ.", "Cho vào máy xay sinh tố, xay nhuyễn.", "Lọc qua rây và uống ngay."]'::jsonb,'Cà rốt và củ dền cung cấp nhiều vitamin A và chất xơ giúp nhuận tràng và phòng',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',30,'Chương 4: Tiêu Hóa','Bệnh trĩ',2,'16919949ba9c1a697f1c91d0465e97490bd65cc22f4a5637a57684aa7c3a26ea',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Phi hành tím với dầu, cho nấm vào xào sơ.
Thêm nước, đun sôi rồi cho đậu hũ vào nấu chín.
Nêm nếm lại cho vừa miệng.',0,0,0,0,0,0,'c04_ung_thu_truc_trang','Ung thư trực tràng','Ung thư trực tràng là tình trạng các tế bào ác tính phát triển trong trực tràng. Chế độ ăn uống giàu chất chống oxy hóa và tăng cường hệ miễn dịch có thể giúp hỗ trợ điều trị.',4,'Tiêu Hóa','["200g nấm hương", "200g đậu hũ", "1 củ hành tím", "Gia vị: muối, tiêu, dầu ăn"]'::jsonb,'["Nấm hương ngâm nở, rửa sạch.", "Đậu hũ cắt miếng vừa ăn.", "Phi hành tím với dầu, cho nấm vào xào sơ.", "Thêm nước, đun sôi rồi cho đậu hũ vào nấu chín.", "Nêm nếm lại cho vừa miệng."]'::jsonb,'Nấm hương có tính kháng viêm, đậu hũ giàu protein giúp tăng cường hệ miễn dịch.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',31,'Chương 4: Tiêu Hóa','Ung thư trực tràng',2,'f240376b9b6f3efa0c3bdbb13141367724f4d1ac3c475263e27f7c4be52ba914',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Nêm nếm lại cho vừa miệng.',0,0,0,0,0,0,'c05_benh_tieu_duong','Bệnh tiểu đường','Bệnh tiểu đường là tình trạng cơ thể không sản xuất đủ insulin hoặc không sử dụng insulin hiệu quả, dẫn đến lượng đường trong máu cao. Chế độ ăn uống ít đường và giàu chất xơ có thể giúp kiểm soát bệnh.',5,'Nội Tiết Và Chuyển Hóa','["300g bí đao", "100g nấm hương", "1 củ hành tím", "Gia vị: muối, tiêu, dầu ăn"]'::jsonb,'["Bí đao gọt vỏ, cắt miếng vừa ăn.", "Nấm hương ngâm nở, rửa sạch.", "Phi hành tím với dầu, cho nấm vào xào sơ.", "Thêm nước, đun sôi rồi cho bí đao vào nấu chín.", "Nêm nếm lại cho vừa miệng."]'::jsonb,'Bí đao và nấm hương giàu chất xơ, giúp kiểm soát lượng đường trong máu.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',32,'Chương 5: Nội Tiết Và Chuyển Hóa','Bệnh tiểu đường',2,'b07e384487d441bf54e506706171c2f5dfa22f7f7356be57b552583fb63d8374',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Cho rong biển vào đun thêm 5 phút.',0,0,0,0,0,0,'c05_benh_mo_mau','Bệnh Mỡ Máu','Bệnh mỡ máu là tình trạng rối loạn chuyển hóa lipid, khiến nồng độ cholesterol xấu (LDL) và triglyceride trong máu tăng cao, đồng thời giảm cholesterol tốt (HDL). Chế độ ăn ít chất béo bão hòa, giàu chất xơ và omega-3 có thể giúp kiểm soát bệnh hiệu quả.',5,'Nội Tiết Và Chuyển Hóa','["50g đậu đen", "10g rong biển khô", "1 củ hành tím", "500ml nước dùng rau củ"]'::jsonb,'["Đậu đen ngâm nước 4 tiếng, rửa sạch.", "Rong biển ngâm nở, rửa sạch.", "Phi thơm hành tím, cho đậu đen vào xào sơ.", "Thêm nước dùng, ninh nhừ đậu.", "Cho rong biển vào đun thêm 5 phút."]'::jsonb,'Đậu đen giàu chất xơ hòa tan giúp giảm cholesterol, rong biển chứa alginate hỗ trợ đào thải mỡ thừa.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',33,'Chương 5: Nội Tiết Và Chuyển Hóa','Bệnh Mỡ Máu',1,'fe7b922cfdfabcec4464d195cde0ed083abfc8f9117754fa97aa133885d97f80',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Cho gừng vào nước sôi, hãm trong 10 phút.
Thêm mật ong, khuấy đều.
Uống ngay.',0,0,0,0,0,0,'c05_benh_gut','Bệnh gút','Bệnh gút là tình trạng viêm khớp do sự tích tụ axit uric trong cơ thể. Chế độ ăn uống ít purin và giàu chất chống viêm có thể giúp cải thiện tình trạng này.',5,'Nội Tiết Và Chuyển Hóa','["1 củ gừng nhỏ", "1 thìa mật ong", "200ml nước sôi"]'::jsonb,'["Gừng rửa sạch, thái lát.", "Cho gừng vào nước sôi, hãm trong 10 phút.", "Thêm mật ong, khuấy đều.", "Uống ngay."]'::jsonb,'Gừng có tính kháng viêm, mật ong giúp tăng cường hệ miễn dịch, hỗ trợ điều trị bệnh gút.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',34,'Chương 5: Nội Tiết Và Chuyển Hóa','Bệnh gút',3,'edecb55360323c47daf871fc6fe33ce7afe8e4acb07cb606122e1e1fa50ac31a',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Thêm gừng đập dập khi gần chín',0,0,0,0,0,0,'c05_viem_than_cap','Viêm thận cấp','Viêm thận cấp là tình trạng viêm nhiễm cấp tính ở thận, thường do nhiễm khuẩn hoặc độc tố. Chế độ ăn uống nhẹ nhàng và giàu chất dinh dưỡng có thể giúp hỗ trợ điều trị.',5,'Nội Tiết Và Chuyển Hóa','["200g bí đỏ", "30g hạt sen tươi", "1 nhánh gừng nhỏ", "500ml nước lọc"]'::jsonb,'["Bí đỏ gọt vỏ, cắt miếng", "Hạt sen bỏ tâm, rửa sạch", "Cho bí đỏ, hạt sen vào nồi ninh nhừ", "Thêm gừng đập dập khi gần chín"]'::jsonb,'Bí đỏ giàu beta-carotene giúp phục hồi tế bào thận, Hạt sen có tác dụng an thần, giảm phù nề, Gừng tăng tuần hoàn máu đến thận',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',35,'Chương 5: Nội Tiết Và Chuyển Hóa','Viêm thận cấp',1,'752f1407e096d0b5da0a1d8b0ce2b5e58d8c28c3a8f918268939d32fd549e17e',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Nêm nhạt bằng chút muối biển',0,0,0,0,0,0,'c05_viem_than_cap','Viêm thận cấp','Viêm thận cấp là tình trạng viêm nhiễm cấp tính ở thận, thường do nhiễm khuẩn hoặc độc tố. Chế độ ăn uống nhẹ nhàng và giàu chất dinh dưỡng có thể giúp hỗ trợ điều trị.',5,'Nội Tiết Và Chuyển Hóa','["50g gạo lứt", "30g đậu đen (đã ngâm)", "1 củ hành tím", "700ml nước"]'::jsonb,'["Gạo lứt và đậu đen ngâm 4 tiếng", "Phi thơm hành tím, cho gạo và đậu vào xào sơ", "Thêm nước ninh nhừ thành cháo", "Nêm nhạt bằng chút muối biển"]'::jsonb,'Đậu đen giàu anthocyanin chống viêm thận, Gạo lứt cung cấp vitamin nhóm B,',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',35,'Chương 5: Nội Tiết Và Chuyển Hóa','Viêm thận cấp',2,'c975e8eef69713062de3a955079fe4f83f24ae8bd1ebdd7b957a404d4f906f9f',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Xay nhuyễn, lọc qua rây nếu cần',0,0,0,0,0,0,'c05_viem_than_cap','Viêm thận cấp','Viêm thận cấp là tình trạng viêm nhiễm cấp tính ở thận, thường do nhiễm khuẩn hoặc độc tố. Chế độ ăn uống nhẹ nhàng và giàu chất dinh dưỡng có thể giúp hỗ trợ điều trị.',5,'Nội Tiết Và Chuyển Hóa','["200g dưa hấu (bỏ hạt)", "5 lá bạc hà", "1 thìa nước cốt chanh", "100ml nước ấm"]'::jsonb,'["Dưa hấu cắt miếng nhỏ", "Cho vào máy xay cùng bạc hà, nước cốt chanh", "Xay nhuyễn, lọc qua rây nếu cần"]'::jsonb,'Dưa hấu lợi tiểu nhẹ, giảm phù, Bạc hà giúp giải độc thận, Chanh cân bằng điện giải',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',35,'Chương 5: Nội Tiết Và Chuyển Hóa','Viêm thận cấp',3,'2062e2c8114fa39709adf28dd7be0622b4d51696d610bf30e80a89eab3e41e3c',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Phi hành tím với dầu, cho cà rốt vào xào sơ.
Thêm nước, đun sôi rồi cho rau cần tây vào nấu chín.
Nêm nếm lại cho vừa miệng.',0,0,0,0,0,0,'c05_soi_than','Sỏi thận','Sỏi thận là tình trạng các tinh thể khoáng chất tích tụ trong thận, gây đau và khó chịu. Chế độ ăn uống giàu chất xơ và uống nhiều nước có thể giúp ngăn ngừa và hỗ trợ điều trị sỏi thận.',5,'Nội Tiết Và Chuyển Hóa','["200g rau cần tây", "1 củ cà rốt", "1 củ hành tím", "Gia vị: muối, tiêu, dầu ăn"]'::jsonb,'["Rau cần tây rửa sạch, cắt khúc.", "Cà rốt gọt vỏ, thái lát.", "Phi hành tím với dầu, cho cà rốt vào xào sơ.", "Thêm nước, đun sôi rồi cho rau cần tây vào nấu chín.", "Nêm nếm lại cho vừa miệng."]'::jsonb,'Rau cần tây và cà rốt giàu chất xơ, giúp đào thải độc tố và ngăn ngừa sỏi thận.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',36,'Chương 5: Nội Tiết Và Chuyển Hóa','Sỏi thận',2,'ad1d891c49b0bda3a9392c7814b653e571f30e3436f627faff4a2027907b69ec',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Lọc bỏ bã, có thể thêm mật ong.',0,0,0,0,0,0,'c05_soi_than','Sỏi thận','Sỏi thận là tình trạng các tinh thể khoáng chất tích tụ trong thận, gây đau và khó chịu. Chế độ ăn uống giàu chất xơ và uống nhiều nước có thể giúp ngăn ngừa và hỗ trợ điều trị sỏi thận.',5,'Nội Tiết Và Chuyển Hóa','["3 lát cam thảo", "1 thanh quế nhỏ 200ml nước sôi"]'::jsonb,'["Đun sôi nước, cho cam thảo và quế vào hãm 5 phút.", "Lọc bỏ bã, có thể thêm mật ong."]'::jsonb,'Gừng có tính kháng viêm, mật ong giúp tăng cường hệ miễn dịch, hỗ trợ điều trị sỏi thận.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',36,'Chương 5: Nội Tiết Và Chuyển Hóa','Sỏi thận',3,'b8da61c2cd0cc9b76578f290cbdf018ae6fcdc40477425048e5d0da3dc9b76fd',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Cho gà, kỷ tử, táo đỏ và gừng vào nồi hầm.
Hầm đến khi gà chín mềm.
Nêm nếm lại cho vừa miệng.',0,0,0,0,0,0,'c06_dau_bung_kinh','Đau bụng kinh','Đau bụng kinh là tình trạng phổ biến ở phụ nữ trong chu kỳ kinh nguyệt. Chế độ ăn uống giàu chất dinh dưỡng và thư giãn có thể giúp giảm triệu chứng.',6,'Phụ Khoa','["1 con gà ác", "50g kỷ tử", "50g táo đỏ", "1 củ gừng", "Gia vị: muối, tiêu"]'::jsonb,'["Gà làm sạch, chặt miếng vừa ăn.", "Cho gà, kỷ tử, táo đỏ và gừng vào nồi hầm.", "Hầm đến khi gà chín mềm.", "Nêm nếm lại cho vừa miệng."]'::jsonb,'Gà hầm thuốc Bắc giúp bồi bổ cơ thể, tăng cường lưu thông máu và giảm đau bụng kinh.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',38,'Chương 6: Phụ Khoa','Đau bụng kinh',2,'22218aa3b6ab5269659307585f9fe676e173b3000f3e12bb8ac298ee0a379108',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Cho chuối, hạt điều, sữa hạnh nhân và mật ong vào máy xay sinh tố.
Xay nhuyễn và dùng ngay.',0,0,0,0,0,0,'c06_be_kinh','Bế kinh','Bế kinh là tình trạng kinh nguyệt không xuất hiện trong một thời gian dài, thường do rối loạn nội tiết. Chế độ ăn uống giàu chất dinh dưỡng và tăng cường lưu thông máu có thể giúp cải thiện tình trạng này.',6,'Phụ Khoa','["1 quả chuối", "50g hạt điều", "200ml sữa hạnh nhân", "1 thìa mật ong"]'::jsonb,'["Chuối bóc vỏ, cắt nhỏ.", "Cho chuối, hạt điều, sữa hạnh nhân và mật ong vào máy xay sinh tố.", "Xay nhuyễn và dùng ngay."]'::jsonb,'Chuối và hạt điều giàu chất dinh dưỡng, giúp tăng cường lưu thông máu và cải thiện bế kinh.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',39,'Chương 6: Phụ Khoa','Bế kinh',1,'1dfc1d9400453cde9e323ce631c1028a9b3a82882fa80160d8589d02a4d42d46',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Thêm mật ong, khuấy đều.
Uống ngay.',0,0,0,0,0,0,'c06_be_kinh','Bế kinh','Bế kinh là tình trạng kinh nguyệt không xuất hiện trong một thời gian dài, thường do rối loạn nội tiết. Chế độ ăn uống giàu chất dinh dưỡng và tăng cường lưu thông máu có thể giúp cải thiện tình trạng này.',6,'Phụ Khoa','["1 lát nhân sâm tươi", "1 thìa mật ong", "200ml nước sôi"]'::jsonb,'["Cho nhân sâm vào nước sôi, hãm trong 10 phút.", "Thêm mật ong, khuấy đều.", "Uống ngay."]'::jsonb,'Nhân sâm có tác dụng tăng cường sinh lực, mật ong giúp tăng cường sức đề kháng, hỗ trợ cải thiện bế kinh.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',40,'Chương 6: Phụ Khoa','Bế kinh',3,'147de166672aabdfd62c07c6331bc41a91a807862a86153f49f0d356295c6695',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Phi hành tím với dầu, cho nấm vào xào sơ.
Thêm nước, đun sôi rồi cho đậu hũ vào nấu chín.
Nêm nếm lại cho vừa miệng.',0,0,0,0,0,0,'c06_viem_am_dao','Viêm âm đạo','Viêm âm đạo là tình trạng viêm nhiễm ở âm đạo, thường do vi khuẩn hoặc nấm gây ra. Chế độ ăn uống giàu chất chống oxy hóa và tăng cường hệ miễn dịch có thể giúp hỗ trợ điều trị.',6,'Phụ Khoa','["200g nấm hương", "200g đậu hũ", "1 củ hành tím", "Gia vị: muối, tiêu, dầu ăn"]'::jsonb,'["Nấm hương ngâm nở, rửa sạch.", "Đậu hũ cắt miếng vừa ăn.", "Phi hành tím với dầu, cho nấm vào xào sơ.", "Thêm nước, đun sôi rồi cho đậu hũ vào nấu chín.", "Nêm nếm lại cho vừa miệng."]'::jsonb,'Nấm hương có tính kháng viêm, đậu hũ giàu protein giúp tăng cường hệ miễn dịch.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',40,'Chương 6: Phụ Khoa','Viêm âm đạo',2,'8495ebd4021b174b7d0bcd4d3fbe9f5ef9cb3b196abcb36a2d14d5740927dcc3',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Thêm mật ong, khuấy đều.
Uống ngay.',0,0,0,0,0,0,'c06_khi_hu_bat_thuong','Khí hư bất thường','Khí hư bất thường là tình trạng dịch tiết âm đạo có màu sắc, mùi hoặc lượng bất thường. Chế độ ăn uống giàu chất dinh dưỡng và tăng cường hệ miễn dịch có thể giúp cải thiện tình trạng này.',6,'Phụ Khoa','["1 lát nhân sâm tươi", "1 thìa mật ong", "200ml nước sôi"]'::jsonb,'["Cho nhân sâm vào nước sôi, hãm trong 10 phút.", "Thêm mật ong, khuấy đều.", "Uống ngay."]'::jsonb,'Nhân sâm có tác dụng tăng cường sinh lực, mật ong giúp tăng cường sức đề kháng, hỗ trợ cải thiện khí hư bất thường.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',42,'Chương 6: Phụ Khoa','Khí hư bất thường',3,'3854003bbce51b73b9b1b88e435ca1820087ddab68cd3f7cc82a74d0d7db2b6f',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Thêm nước vào nồi, đun sôi rồi cho rau cần vào.
Nêm gia vị vừa ăn.',0,0,0,0,0,0,'c06_viem_co_tu_cung','Viêm cổ tử cung','Viêm cổ tử cung là tình trạng viêm nhiễm ở cổ tử cung, gây đau, chảy máu bất thường và có thể gây ảnh hưởng đến khả năng sinh sản. Nguyên nhân: Nhiễm trùng do vi khuẩn hoặc virus (thường là lây qua quan hệ tình dục), sử dụng các sản phẩm không phù hợp, hoặc vệ sinh không đúng cách.',6,'Phụ Khoa','["200g cá chép", "100g rau cần", "Gia vị: muối, tiêu"]'::jsonb,'["Cá chép làm sạch, cắt khúc vừa ăn.", "Rau cần rửa sạch, cắt khúc.", "Phi hành tím với dầu ăn cho thơm, cho cá vào xào sơ.", "Thêm nước vào nồi, đun sôi rồi cho rau cần vào.", "Nêm gia vị vừa ăn."]'::jsonb,'Cá chép cung cấp omega-3 giúp cải thiện sức khỏe sinh sản, rau cần giúp làm mát và thanh nhiệt cơ thể.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',44,'Chương 6: Phụ Khoa','Viêm cổ tử cung',1,'ffd850349b360eea27d4a3c7b7b63ca373aaf9ecc553f333fd26ff2e7f72b6c4',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Thêm mật ong vào và khuấy đều.',0,0,0,0,0,0,'c06_viem_co_tu_cung','Viêm cổ tử cung','Viêm cổ tử cung là tình trạng viêm nhiễm ở cổ tử cung, gây đau, chảy máu bất thường và có thể gây ảnh hưởng đến khả năng sinh sản. Nguyên nhân: Nhiễm trùng do vi khuẩn hoặc virus (thường là lây qua quan hệ tình dục), sử dụng các sản phẩm không phù hợp, hoặc vệ sinh không đúng cách.',6,'Phụ Khoa','["1 nhánh gừng tươi", "1 thìa mật ong", "Nước sôi"]'::jsonb,'["Gừng thái lát mỏng.", "Đun nước sôi, cho gừng vào, để ngấm trong 5 phút.", "Thêm mật ong vào và khuấy đều."]'::jsonb,'Gừng giúp làm dịu viêm, hỗ trợ điều trị viêm nhiễm, mật ong giúp giảm sưng viêm và tăng cường sức đề kháng.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',44,'Chương 6: Phụ Khoa','Viêm cổ tử cung',2,'d3e185f50bf1b5f98cfe12e5441f2a71f7d61ff0c809d0d8f36eb1e73b1d64c1',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Nêm gia vị vừa ăn.',0,0,0,0,0,0,'c06_vo_sinh','Vô sinh','Vô sinh là tình trạng không thể thụ thai sau một năm quan hệ tình dục đều đặn mà không sử dụng biện pháp tránh thai. Nguyên nhân có thể do các vấn đề ở cả nam và nữ. Nguyên nhân: Rối loạn nội tiết tố, viêm nhiễm sinh dục, vấn đề về ống dẫn trứng, tinh trùng yếu hoặc không đủ.',6,'Phụ Khoa','["200g thịt gà", "100g rau ngót", "Gia vị: muối, tiêu, dầu ăn"]'::jsonb,'["Thịt gà làm sạch, thái miếng vừa ăn.", "Rau ngót rửa sạch.", "Phi hành tím với dầu ăn cho thơm, cho thịt gà vào xào sơ.", "Thêm nước vào nồi, đun sôi rồi cho rau ngót vào.", "Nêm gia vị vừa ăn."]'::jsonb,'Thịt gà cung cấp protein giúp duy trì sức khỏe, rau ngót giúp thanh nhiệt và cải thiện chức năng sinh sản.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',45,'Chương 6: Phụ Khoa','Vô sinh',1,'01bb8124786451d4287e0e14dadfcbc198c9b40b1fc5e3c07f9b3b1c3280abc7',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Cho vào máy xay sinh tố, xay nhuyễn.
Thêm mật ong vào và khuấy đều.',0,0,0,0,0,0,'c06_vo_sinh','Vô sinh','Vô sinh là tình trạng không thể thụ thai sau một năm quan hệ tình dục đều đặn mà không sử dụng biện pháp tránh thai. Nguyên nhân có thể do các vấn đề ở cả nam và nữ. Nguyên nhân: Rối loạn nội tiết tố, viêm nhiễm sinh dục, vấn đề về ống dẫn trứng, tinh trùng yếu hoặc không đủ.',6,'Phụ Khoa','["1 củ cà rốt", "1/2 quả dứa", "1 ít mật ong"]'::jsonb,'["Cà rốt và dứa gọt vỏ, cắt miếng nhỏ.", "Cho vào máy xay sinh tố, xay nhuyễn.", "Thêm mật ong vào và khuấy đều."]'::jsonb,'Cà rốt và dứa cung cấp vitamin A và C, hỗ trợ tăng cường sức khỏe sinh sản và cải thiện khả năng thụ thai.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',45,'Chương 6: Phụ Khoa','Vô sinh',2,'3be2afd420ea43683aa17b63ed4ae6d98473845a9fdd91001866c67470eba2e1',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Phi hành tím với dầu, cho hàu vào xào sơ.
Thêm nước, đun sôi rồi cho rau cần tây vào nấu chín.
Nêm nếm lại cho vừa miệng.',0,0,0,0,0,0,'c07_roi_loan_cuong_duong','Rối loạn cương dương','Rối loạn cương dương là tình trạng không thể duy trì sự cương cứng đủ để quan hệ tình dục. Chế độ ăn uống giàu chất dinh dưỡng và tăng cường lưu thông máu có thể giúp cải thiện tình trạng này.',7,'Nam Khoa','["200g hàu tươi", "200g rau cần tây", "1 củ hành tím", "Gia vị: muối, tiêu, dầu ăn"]'::jsonb,'["Hàu rửa sạch, bỏ vỏ.", "Rau cần tây rửa sạch, cắt khúc.", "Phi hành tím với dầu, cho hàu vào xào sơ.", "Thêm nước, đun sôi rồi cho rau cần tây vào nấu chín.", "Nêm nếm lại cho vừa miệng."]'::jsonb,'Hàu giàu kẽm, rau cần tây giàu chất xơ, giúp tăng cường sinh lý và lưu thông máu.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',48,'Chương 7: Nam Khoa','Rối loạn cương dương',2,'758198cd7737f9b26a4d245fcc87f9bbe2bca6343e11fe5ef2754e2cbb5075d6',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Thêm mật ong, khuấy đều.
Uống ngay.',0,0,0,0,0,0,'c07_roi_loan_cuong_duong','Rối loạn cương dương','Rối loạn cương dương là tình trạng không thể duy trì sự cương cứng đủ để quan hệ tình dục. Chế độ ăn uống giàu chất dinh dưỡng và tăng cường lưu thông máu có thể giúp cải thiện tình trạng này.',7,'Nam Khoa','["1 lát nhân sâm tươi", "1 thìa mật ong", "200ml nước sôi"]'::jsonb,'["Cho nhân sâm vào nước sôi, hãm trong 10 phút.", "Thêm mật ong, khuấy đều.", "Uống ngay."]'::jsonb,'Nhân sâm có tác dụng tăng cường sinh lực, mật ong giúp tăng cường sức đề kháng, hỗ trợ điều trị rối loạn cương dương.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',48,'Chương 7: Nam Khoa','Rối loạn cương dương',3,'1457fbe2bf29f6502492fa537eeff5cc9cf953cee7a04de7c0626045d8969ed5',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Nêm gia vị, thêm hành lá, rau mùi, dùng khi còn ấm.',0,0,0,0,0,0,'c07_xuat_tinh_som','Xuất tinh sớm','Xuất tinh sớm là tình trạng nam giới xuất tinh trong thời gian quá ngắn sau khi bắt đầu quan hệ tình dục, dẫn đến sự không thoải mái và lo âu cho cả hai bên. Nguyên nhân: Căng thẳng, lo âu, rối loạn hormone, hoặc các vấn đề tâm lý và thể chất.',7,'Nam Khoa','["5-6 con hàu tươi", "½ chén gạo tẻ", "10 hạt sen tươi (hoặc hạt sen khô ngâm mềm)", "1 củ hành tím băm nhỏ", "1 ít hành lá, rau mùi", "Gia vị: muối, tiêu, nước mắm"]'::jsonb,'["Hàu tách vỏ, rửa sạch, cắt nhỏ.", "Hạt sen nấu với gạo tẻ cho đến khi cháo nhừ.", "Phi thơm hành tím, cho hàu vào xào nhanh rồi đổ vào cháo.", "Nêm gia vị, thêm hành lá, rau mùi, dùng khi còn ấm."]'::jsonb,'Đậu hũ chứa nhiều protein giúp cải thiện sức khỏe sinh lý, rau mồng tơi giúp giải độc, thanh nhiệt cơ thể, hỗ trợ làm dịu thần kinh.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',49,'Chương 7: Nam Khoa','Xuất tinh sớm',1,'7654848e9d2a9db879b88191cbe0cde5eb0c057e97b3a20795afedee03e6654e',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Thêm thịt bò, đun chín mềm, nêm gia vị vừa ăn.',0,0,0,0,0,0,'c07_xuat_tinh_som','Xuất tinh sớm','Xuất tinh sớm là tình trạng nam giới xuất tinh trong thời gian quá ngắn sau khi bắt đầu quan hệ tình dục, dẫn đến sự không thoải mái và lo âu cho cả hai bên. Nguyên nhân: Căng thẳng, lo âu, rối loạn hormone, hoặc các vấn đề tâm lý và thể chất.',7,'Nam Khoa','["200g thịt bò thăn", "10g ngọc trúc (hoặc hoài sơn khô)", "5g kỷ tử", "1 củ hành tím băm nhỏ", "500ml nước lọc", "Gia vị: muối, tiêu, nước mắm"]'::jsonb,'["Ngọc trúc và kỷ tử rửa sạch, ngâm nước 10 phút.", "Thịt bò thái lát mỏng, ướp muối tiêu.", "Đun sôi nước, cho ngọc trúc và kỷ tử vào nấu 15 phút.", "Thêm thịt bò, đun chín mềm, nêm gia vị vừa ăn."]'::jsonb,'Ngọc trúc giúp bổ thận, tăng cường sinh lực, hỗ trợ cải thiện xuất tinh sớm. Kỷ tử giúp dưỡng huyết, tăng sức bền, cải thiện chất lượng quan hệ.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',49,'Chương 7: Nam Khoa','Xuất tinh sớm',2,'13f15293e54e595e065cf840612938378d0cd5716c9584069f31580642dc8f46',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Nêm gia vị vừa ăn, dùng nóng.',0,0,0,0,0,0,'c07_di_tinh','Di tinh','Di tinh là tình trạng xuất tinh không tự chủ trong khi ngủ hoặc khi không có kích thích tình dục. Đây có thể là dấu hiệu của sự mất cân bằng nội tiết hoặc vấn đề về tâm lý. Nguyên nhân: Căng thẳng, lo âu, mệt mỏi, rối loạn nội tiết tố.',7,'Nam Khoa','["½ chén gạo tẻ", "1 quả trứng gà ta", "1 ít lá tía tô", "1 nhánh hành lá", "Gia vị: muối, nước mắm, tiêu"]'::jsonb,'["Nấu cháo từ gạo tẻ cho đến khi nhừ.", "Đập trứng gà vào, khuấy đều, nấu thêm 2 phút.", "Cho tía tô và hành lá thái nhỏ vào, tắt bếp.", "Nêm gia vị vừa ăn, dùng nóng."]'::jsonb,'Cháo giúp cơ thể ấm, tía tô hỗ trợ lưu thông khí huyết, giúp giảm căng thẳng – một nguyên nhân gây di tinh.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',50,'Chương 7: Nam Khoa','Di tinh',2,'a58b653e79ae8de6abf988323336ef3cf224b6d7be00ea34ae02fb59896dc6b3',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Uống ngay.',0,0,0,0,0,0,'c08_viem_loet_mieng','Viêm loét miệng','Viêm loét miệng là tình trạng xuất hiện các vết loét trong miệng, gây đau và khó chịu. Chế độ ăn uống giàu vitamin và khoáng chất có thể giúp làm lành vết loét.',8,'Tai Mũi Họng','["1 củ gừng nhỏ", "1 quả chanh", "200ml nước sôi"]'::jsonb,'["Gừng rửa sạch, thái lát.", "Chanh vắt lấy nước cốt.", "Cho gừng vào nước sôi, hãm trong 10 phút.", "Thêm nước cốt chanh, khuấy đều.", "Uống ngay."]'::jsonb,'Gừng có tính kháng viêm, chanh giàu vitamin C, giúp làm lành vết loét miệng.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',53,'Chương 8: Tai Mũi Họng','Viêm loét miệng',3,'211abf8352a33e1ea4090619e09fd85d0f3bf398ba8dc096c40bf7d3a0f393ea',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Thêm mật ong vào khuấy đều.',0,0,0,0,0,0,'c08_viem_mui','Viêm mũi','Viêm mũi là tình trạng viêm nhiễm ở niêm mạc mũi, gây tắc nghẽn mũi, chảy nước mũi và khó thở. Bệnh có thể do vi khuẩn, virus hoặc các dị ứng gây ra.',8,'Tai Mũi Họng','["1 nhánh gừng tươi", "1 thìa mật ong", "Nước sôi"]'::jsonb,'["Gừng cạo vỏ, thái lát mỏng.", "Đun nước sôi, cho gừng vào hãm trong 5 phút.", "Thêm mật ong vào khuấy đều."]'::jsonb,'Gừng giúp làm dịu viêm, mật ong hỗ trợ làm sạch mũi, giúp giảm tắc nghẽn và triệu chứng viêm mũi.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',54,'Chương 8: Tai Mũi Họng','Viêm mũi',1,'e3957aa9776e0cbb5ff5251480e7bf448aff81162304b2d0a4838c7a2324e055',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Cho vào máy xay sinh tố, xay nhuyễn.
Lọc qua rây và uống ngay.',0,0,0,0,0,0,'c08_viem_mui','Viêm mũi','Viêm mũi là tình trạng viêm nhiễm ở niêm mạc mũi, gây tắc nghẽn mũi, chảy nước mũi và khó thở. Bệnh có thể do vi khuẩn, virus hoặc các dị ứng gây ra.',8,'Tai Mũi Họng','["1 củ cà rốt", "1/2 quả dứa"]'::jsonb,'["Cà rốt và dứa gọt vỏ, cắt miếng nhỏ.", "Cho vào máy xay sinh tố, xay nhuyễn.", "Lọc qua rây và uống ngay."]'::jsonb,'Cà rốt và dứa giúp tăng cường hệ miễn dịch, hỗ trợ giảm viêm, giải độc cơ thể.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',54,'Chương 8: Tai Mũi Họng','Viêm mũi',2,'f5306dee1684270d626c5aab3e1eb42cb75c1410b1a16a47e38426202cdb8fe7',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Lá bạc hà rửa sạch.
Cho tất cả vào máy xay sinh tố, xay nhuyễn.
Lọc qua rây và thưởng thức ngay.',0,0,0,0,0,0,'c08_u_tai_diec_tai','Ù tai, điếc tai','Ù tai và điếc tai là tình trạng khi người bệnh nghe thấy âm thanh trong tai mà không có nguồn âm thanh bên ngoài. Các triệu chứng đi kèm có thể bao gồm cảm giác tắc nghẽn tai, khó nghe và chóng mặt. Nguyên nhân: Căng thẳng, tiếp xúc với tiếng ồn lớn, tắc nghẽn tai, viêm nhiễm hoặc tổn thương thần kinh thính giác.',8,'Tai Mũi Họng','["200g dưa hấu", "10 lá bạc hà"]'::jsonb,'["Dưa hấu gọt vỏ, cắt miếng nhỏ.", "Lá bạc hà rửa sạch.", "Cho tất cả vào máy xay sinh tố, xay nhuyễn.", "Lọc qua rây và thưởng thức ngay."]'::jsonb,'Dưa hấu giúp giải độc cơ thể, làm mát, trong khi bạc hà giúp thư giãn thần kinh và làm dịu các triệu chứng ù tai.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',54,'Chương 8: Tai Mũi Họng','Ù tai, điếc tai',1,'86c6a3242660d4128ac877539b25ed106e8951f2a7798ae720986c2598d87018',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Cho rau ngót vào, nấu chín mềm, nêm gia vị vừa ăn.',0,0,0,0,0,0,'c08_viem_ket_mac_truyen_nhiem','Viêm kết mạc truyền nhiễm','Viêm kết mạc truyền nhiễm là tình trạng viêm nhiễm ở kết mạc mắt, gây đỏ mắt, ngứa, chảy nước mắt và có thể có mủ. Tình trạng này có thể lây qua tiếp xúc với vi khuẩn, virus hoặc chất kích thích.',8,'Tai Mũi Họng','["1 bó rau ngót", "100g thịt nạc băm", "1 củ hành tím", "Gia vị: muối, tiêu, nước mắm"]'::jsonb,'["Rau ngót rửa sạch, vò nhẹ để ra chất dinh dưỡng.", "Phi hành tím, xào thịt băm, thêm nước nấu sôi.", "Cho rau ngót vào, nấu chín mềm, nêm gia vị vừa ăn."]'::jsonb,'Rau ngót giàu vitamin C giúp tăng cường hệ miễn dịch, giảm viêm nhiễm, hỗ trợ mắt khỏe mạnh.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',56,'Chương 8: Tai Mũi Họng','Viêm kết mạc truyền nhiễm',2,'fc1eddb54a0bdae6250f04d6c92a3da58aedcd693c8ed5635dbf9163b73d88ba',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Thêm đậu phụ, nấu 5 phút, nêm gia vị vừa ăn.',0,0,0,0,0,0,'c09_nam_da','Nám da','Nám da là tình trạng xuất hiện các đốm nâu hoặc đen trên da, thường gặp ở vùng má, trán, mũi và cằm. Bệnh không gây nguy hiểm nhưng ảnh hưởng đến thẩm mỹ. Nám da thường do sự thay đổi nội tiết tố, đặc biệt là trong thai kỳ, sử dụng thuốc tránh thai, hoặc do tác động của ánh nắng mặt trời. Nguyên nhân: Tăng sắc tố do ánh nắng mặt trời, thay đổi nội tiết tố, di truyền, sử dụng mỹ phẩm không phù hợp, căng thẳng.',9,'Bệnh Da Liễu','["2 quả cà chua", "1 miếng đậu phụ", "1 ít hành lá", "Gia vị: muối, tiêu, nước mắm"]'::jsonb,'["Cà chua rửa sạch, cắt múi cau.", "Đậu phụ cắt miếng vừa ăn.", "Phi hành, xào cà chua, đổ nước vào nấu sôi.", "Thêm đậu phụ, nấu 5 phút, nêm gia vị vừa ăn."]'::jsonb,'Cà chua giàu lycopene giúp chống oxy hóa, bảo vệ da khỏi tác động của tia UV. Đậu phụ chứa isoflavone giúp cân bằng nội tiết tố, hỗ trợ làm mờ nám.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',59,'Chương 9: Bệnh Da Liễu','Nám da',3,'ba365b2f3401225902cd0c7243b2225efc9cecb0466a96255a7de7bc46bb4a36',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Phi hành với dầu ăn cho thơm, cho tôm vào xào sơ.
Thêm nước vào nồi, đun sôi rồi cho củ cải vào.
Nêm gia vị vừa ăn.',0,0,0,0,0,0,'c09_te_cong','Tê cóng','Tê cóng là cảm giác tê bì, lạnh ở tay, chân, đặc biệt là khi cơ thể tiếp xúc với nhiệt độ lạnh. Tình trạng này có thể xuất hiện do thiếu máu, rối loạn tuần hoàn máu, hoặc mắc các bệnh về thần kinh. Nguyên nhân: thiếu hụt vitamin B12, căng thẳng hoặc tiếp xúc lâu dài với lạnh.',9,'Bệnh Da Liễu','["200g tôm", "100g củ cải", "Gia vị: muối, tiêu"]'::jsonb,'["Tôm làm sạch, củ cải gọt vỏ, cắt miếng vừa ăn.", "Phi hành với dầu ăn cho thơm, cho tôm vào xào sơ.", "Thêm nước vào nồi, đun sôi rồi cho củ cải vào.", "Nêm gia vị vừa ăn."]'::jsonb,'Củ cải giúp lưu thông khí huyết, tôm bổ sung protein giúp cải thiện tuần hoàn máu, giảm tê cóng.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',60,'Chương 9: Bệnh Da Liễu','Tê cóng',1,'66dee2e5336bc7333c7028cea40790a1ee1588d729e52b31e2dfb5b6d1904447',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Uống ngay.',0,0,0,0,0,0,'c09_te_cong','Tê cóng','Tê cóng là cảm giác tê bì, lạnh ở tay, chân, đặc biệt là khi cơ thể tiếp xúc với nhiệt độ lạnh. Tình trạng này có thể xuất hiện do thiếu máu, rối loạn tuần hoàn máu, hoặc mắc các bệnh về thần kinh. Nguyên nhân: thiếu hụt vitamin B12, căng thẳng hoặc tiếp xúc lâu dài với lạnh.',9,'Bệnh Da Liễu','["1 củ cà rốt", "1 củ dền"]'::jsonb,'["Cà rốt và củ dền gọt vỏ, cắt miếng vừa ăn.", "Cho vào máy xay, xay nhuyễn và lọc qua rây.", "Uống ngay."]'::jsonb,'Cà rốt và củ dền giúp tăng cường tuần hoàn máu, bổ sung dưỡng chất giúp cải thiện tình trạng tê cóng.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',60,'Chương 9: Bệnh Da Liễu','Tê cóng',2,'af212c9d8706bf23c6d357198fbd9cf47b48354c22bbcda4a1a446851bad6497',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Thêm mật ong vào và khuấy đều.',0,0,0,0,0,0,'c09_te_cong','Tê cóng','Tê cóng là cảm giác tê bì, lạnh ở tay, chân, đặc biệt là khi cơ thể tiếp xúc với nhiệt độ lạnh. Tình trạng này có thể xuất hiện do thiếu máu, rối loạn tuần hoàn máu, hoặc mắc các bệnh về thần kinh. Nguyên nhân: thiếu hụt vitamin B12, căng thẳng hoặc tiếp xúc lâu dài với lạnh.',9,'Bệnh Da Liễu','["1 nhánh gừng tươi, 1 nhánh quế", "1 thìa mật ong", "Nước sôi"]'::jsonb,'["Gừng thái lát mỏng, quế cho vào cốc.", "Đổ nước sôi vào, để ngấm trong 5 phút.", "Thêm mật ong vào và khuấy đều."]'::jsonb,'Gừng giúp kích thích tuần hoàn máu, làm ấm cơ thể, mật ong hỗ trợ tăng cường hệ miễn dịch và làm dịu cơ thể.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',60,'Chương 9: Bệnh Da Liễu','Tê cóng',3,'0c46b72daa0e636b3c5ac8703b12e149c9c5fca5a2291ff3c71492d51d03831a',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Khi cháo nhuyễn, thêm đường phèn, khuấy đều.',0,0,0,0,0,0,'c10_bieng_an','Biếng ăn','Biếng ăn là tình trạng trẻ không muốn ăn hoặc ăn ít, dẫn đến thiếu hụt dinh dưỡng. Chế độ ăn uống giàu dinh dưỡng và hấp dẫn có thể giúp cải thiện tình trạng này.',10,'Nhi Khoa','["100g khoai lang", "30g hạt điều", "200ml nước", "1 ít đường phèn"]'::jsonb,'["Khoai lang gọt vỏ, cắt nhỏ, nấu chín.", "Hạt điều xay nhuyễn, cho vào nấu cùng khoai lang.", "Khi cháo nhuyễn, thêm đường phèn, khuấy đều."]'::jsonb,'Khoai lang giàu beta-carotene giúp kích thích vị giác, hạt điều cung cấp protein và chất béo tốt.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',61,'Chương 10: Nhi Khoa','Biếng ăn',2,'434d0af72a558d053e2e44feb22923b5abaf6e18603c013ae2539237b7e07db4',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Phi hành tím với dầu, cho thịt gà vào xào sơ.
Thêm nước, đun sôi rồi cho bí đỏ vào nấu chín.
Nêm nếm lại cho vừa miệng.',0,0,0,0,0,0,'c10_bieng_an','Biếng ăn','Biếng ăn là tình trạng trẻ không muốn ăn hoặc ăn ít, dẫn đến thiếu hụt dinh dưỡng. Chế độ ăn uống giàu dinh dưỡng và hấp dẫn có thể giúp cải thiện tình trạng này.',10,'Nhi Khoa','["300g bí đỏ", "200g thịt gà", "1 củ hành tím", "Gia vị: muối, tiêu, dầu ăn"]'::jsonb,'["Bí đỏ gọt vỏ, cắt miếng vừa ăn.", "Thịt gà rửa sạch, thái miếng.", "Phi hành tím với dầu, cho thịt gà vào xào sơ.", "Thêm nước, đun sôi rồi cho bí đỏ vào nấu chín.", "Nêm nếm lại cho vừa miệng."]'::jsonb,'Bí đỏ giàu vitamin A, thịt gà giàu protein, giúp cải thiện tình trạng biếng ăn.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',61,'Chương 10: Nhi Khoa','Biếng ăn',3,'f3a8c726f6ee45b6a2cda0665e71cf35a242a0afed079dfc884ae01a41b24d59',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Thêm bí đỏ, nấu thêm 5 phút, nêm gia vị vừa ăn.',0,0,0,0,0,0,'c10_suy_dinh_duong','Suy dinh dưỡng','Suy dinh dưỡng là tình trạng thiếu hụt dưỡng chất cần thiết, dẫn đến chậm phát triển thể chất và trí tuệ, dễ mắc bệnh. Tình trạng này thường do chế độ ăn thiếu hụt vitamin, khoáng chất và protein. Nguyên nhân: Thiếu chất dinh dưỡng, chế độ ăn uống kém, rối loạn tiêu hóa, hoặc bệnh lý mạn tính.',10,'Nhi Khoa','["½ chén gạo tẻ :", "100g thịt bò", "100g bí đỏ", "1 củ hành tím", "Gia vị: muối, tiêu, nước mắm"]'::jsonb,'["Bí đỏ gọt vỏ, cắt nhỏ.", "Nấu cháo với gạo và nước đến khi nhừ.", "Xào thịt bò với hành tím, sau đó cho vào cháo.", "Thêm bí đỏ, nấu thêm 5 phút, nêm gia vị vừa ăn."]'::jsonb,'Thịt bò giàu sắt và protein giúp tăng cường thể lực, bí đỏ bổ sung vitamin A và chất xơ hỗ trợ tiêu hóa tốt.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',63,'Chương 10: Nhi Khoa','Suy dinh dưỡng',1,'3f6824445c9b92ef14e0e7c8dcdffc1604d9649e462a7457243420a80b9b57f5',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Nấu gạo lứt và đậu xanh trong nước cho đến khi cháo nhừ.
Thêm muối vào vừa ăn.',0,0,0,0,0,0,'c10_suy_dinh_duong','Suy dinh dưỡng','Suy dinh dưỡng là tình trạng thiếu hụt dưỡng chất cần thiết, dẫn đến chậm phát triển thể chất và trí tuệ, dễ mắc bệnh. Tình trạng này thường do chế độ ăn thiếu hụt vitamin, khoáng chất và protein. Nguyên nhân: Thiếu chất dinh dưỡng, chế độ ăn uống kém, rối loạn tiêu hóa, hoặc bệnh lý mạn tính.',10,'Nhi Khoa','["100g gạo lứt", "50g đậu xanh", "1 ít muối"]'::jsonb,'["Gạo lứt và đậu xanh rửa sạch, ngâm qua đêm.", "Nấu gạo lứt và đậu xanh trong nước cho đến khi cháo nhừ.", "Thêm muối vào vừa ăn."]'::jsonb,'Gạo lứt và đậu xanh giúp bổ sung chất xơ và các vi chất dinh dưỡng cần thiết cho cơ thể, hỗ trợ phục hồi sức khỏe cho người bị suy dinh dưỡng.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',63,'Chương 10: Nhi Khoa','Suy dinh dưỡng',2,'95862ca6d9809e66a982981b8da2c8a63c3dd1e5d197e4c5d3665b14e284c834',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Nấu củ cải trong nước cho đến khi mềm, sau đó cho cải bó xôi vào nấu chín.
Nêm gia vị vừa ăn.',0,0,0,0,0,0,'c10_chung_dai_dam','Chứng đái dầm','Chứng đái dầm là tình trạng không kiểm soát được việc tiểu tiện vào ban đêm, phổ biến ở trẻ em nhưng cũng có thể xảy ra ở người lớn. Nguyên nhân: Mắc các vấn đề về thần kinh, tiêu thụ nhiều nước trước khi ngủ, căng thẳng tâm lý, hoặc do di truyền.',10,'Nhi Khoa','["200g củ cải", "100g cải bó xôi", "Gia vị: muối, tiêu"]'::jsonb,'["Củ cải gọt vỏ, cắt miếng nhỏ.", "Rau cải bó xôi rửa sạch, cắt khúc.", "Nấu củ cải trong nước cho đến khi mềm, sau đó cho cải bó xôi vào nấu chín.", "Nêm gia vị vừa ăn."]'::jsonb,'Củ cải và cải bó xôi giúp thanh nhiệt, giải độc, làm dịu cơ thể, hỗ trợ điều trị chứng đái dầm.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',65,'Chương 10: Nhi Khoa','Chứng đái dầm',1,'6d5463065ceea2126c2962a5aba119c1f7514f6b062cc41ad8da566ce378b43f',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Xay nhuyễn và dùng ngay.',0,0,0,0,0,0,'c13_loang_xuong','Loãng xương','Loãng xương là tình trạng xương bị mất mật độ, trở nên giòn và dễ gãy. Chế độ ăn uống giàu canxi và vitamin D có thể giúp cải thiện tình trạng này.',13,'Bệnh Về Xương Khớp','["1 quả chuối", "200ml sữa hạt điều", "1 thìa mật ong"]'::jsonb,'["Chuối bóc vỏ, cắt nhỏ.", "Cho chuối, sữa hạt điều và mật ong vào máy xay sinh tố.", "Xay nhuyễn và dùng ngay."]'::jsonb,'Sữa hạt điều giàu canxi, chuối chứa kali giúp tăng cường sức khỏe xương.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',67,'Chương 13: Bệnh Về Xương Khớp','Loãng xương',2,'77d67b46f77a192177f527e8589702ef42b6133c2ba1a1130f58e0afc0f8ded0',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Cho chuối, hạt điều, sữa hạnh nhân và mật ong vào máy xay sinh tố.
Xay nhuyễn và dùng ngay.',0,0,0,0,0,0,'c13_thoai_hoa_dot_song_co','Thoái hóa đốt sống cổ','Thoái hóa đốt sống cổ là tình trạng các đốt sống cổ bị thoái hóa, gây đau và hạn chế vận động. Chế độ ăn uống giàu canxi và chất chống oxy hóa có thể giúp cải thiện tình trạng này. `',13,'Bệnh Về Xương Khớp','["1 quả chuối", "50g hạt điều", "200ml sữa hạnh nhân", "1 thìa mật ong"]'::jsonb,'["Chuối bóc vỏ, cắt nhỏ.", "Cho chuối, hạt điều, sữa hạnh nhân và mật ong vào máy xay sinh tố.", "Xay nhuyễn và dùng ngay."]'::jsonb,'Chuối và hạt điều giàu kali và magie, giúp tăng cường sức khỏe xương và giảm đau đốt sống cổ.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',69,'Chương 13: Bệnh Về Xương Khớp','Thoái hóa đốt sống cổ',1,'ab7c444c7c38347883a190918cc747c6d7f2313798cf4103536eb898d2464b28',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Phi hành tím với dầu, cho xương vào xào sơ.
Thêm nước, đun sôi rồi cho đu đủ vào hầm đến khi chín mềm.
Nêm nếm lại cho vừa miệng.',0,0,0,0,0,0,'c13_thoai_hoa_dot_song_co','Thoái hóa đốt sống cổ','Thoái hóa đốt sống cổ là tình trạng các đốt sống cổ bị thoái hóa, gây đau và hạn chế vận động. Chế độ ăn uống giàu canxi và chất chống oxy hóa có thể giúp cải thiện tình trạng này. `',13,'Bệnh Về Xương Khớp','["500g xương heo", "1 quả đu đủ xanh", "1 củ hành tím", "Gia vị: muối, tiêu, dầu ăn"]'::jsonb,'["Xương heo rửa sạch, chặt khúc.", "Đu đủ gọt vỏ, cắt miếng vừa ăn.", "Phi hành tím với dầu, cho xương vào xào sơ.", "Thêm nước, đun sôi rồi cho đu đủ vào hầm đến khi chín mềm.", "Nêm nếm lại cho vừa miệng."]'::jsonb,'Xương heo giàu canxi, đu đủ chứa enzyme hỗ trợ tiêu hóa và hấp thụ canxi, giúp cải thiện thoái hóa đốt sống cổ.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',69,'Chương 13: Bệnh Về Xương Khớp','Thoái hóa đốt sống cổ',2,'e78c3ea4297332f153b5a2c4c4b8ca359d4ea27015890a39ec376f93627f7fe2',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();
...
Thêm vừng đen xay nhuyễn và mật ong, khuấy đều, uống ấm.',0,0,0,0,0,0,'c13_benh_tang_sinh_xuong','Bệnh tăng sinh xương','Bệnh tăng sinh xương là tình trạng xương phát triển quá mức, gây đau và hạn chế vận động. Chế độ ăn uống giàu canxi và vitamin D có thể giúp cải thiện tình trạng này.',13,'Bệnh Về Xương Khớp','["100g đậu nành", "50g vừng đen (mè đen)", "500ml nước", "1 thìa mật ong"]'::jsonb,'["Đậu nành ngâm nước 6 tiếng, đãi sạch vỏ.", "Vừng đen rang thơm.", "Xay đậu nành với nước, lọc lấy nước, đun sôi nhẹ.", "Thêm vừng đen xay nhuyễn và mật ong, khuấy đều, uống ấm."]'::jsonb,'Đậu nành và vừng đen giàu canxi, kẽm, giúp nuôi dưỡng xương khớp, làm chậm quá trình tăng sinh xương bất thường.',null,'[]'::jsonb,'[]'::jsonb,'missing_source_data','awaiting_professional_review','source_imported',false,'Suc_Khoe_Tu_Nha_Bep_Thuc_Don_Theo_Tung_Muc.md',71,'Chương 13: Bệnh Về Xương Khớp','Bệnh tăng sinh xương',3,'1551ae246434224892111b0a30dbf4ea5052e5245d857a806742217375e15315',1,true) on conflict (code) do update set meal_type=excluded.meal_type, meal_name=excluded.meal_name, description=excluded.description, cooking_instructions=excluded.cooking_instructions, calories=excluded.calories, protein=excluded.protein, carbs=excluded.carbs, fat=excluded.fat, fiber=excluded.fiber, water_ml=excluded.water_ml, health_topic_code=excluded.health_topic_code, health_topic_name=excluded.health_topic_name, health_topic_description=excluded.health_topic_description, chapter_number=excluded.chapter_number, chapter_name=excluded.chapter_name, ingredients_json=excluded.ingredients_json, cooking_steps_json=excluded.cooking_steps_json, benefits=excluded.benefits, serving_size=excluded.serving_size, allergen_tags_json=excluded.allergen_tags_json, avoid_condition_tags_json=excluded.avoid_condition_tags_json, nutrition_status=excluded.nutrition_status, constraint_metadata_status=excluded.constraint_metadata_status, metadata_status=excluded.metadata_status, is_plan_eligible=excluded.is_plan_eligible, source_name=excluded.source_name, source_page=excluded.source_page, source_chapter=excluded.source_chapter, source_topic=excluded.source_topic, source_recipe_order=excluded.source_recipe_order, source_hash=excluded.source_hash, version=excluded.version, is_active=excluded.is_active, updated_at=now();

-- END GENERATED MEAL CATALOG

-- =============================================================================
-- BEGIN FAIL-FAST SEED VALIDATION
-- =============================================================================
...
begin
  ...
  if exists (
    ...
    where nutrition_status = 'estimated_from_ingredients'
      and (calories <= 0 or nullif(btrim(serving_size), '') is null)
  ) then
    raise exception 'MEAL_NUTRITION_ESTIMATED_REQUIRED_FIELDS_MISSING';
  end if;
  ...
  if exists (
    ...
    raise exception 'MEAL_NUTRITION_NEGATIVE_VALUE';
  end if;
  ...
  if exists (
    ...
    raise exception 'MEAL_NUTRITION_ESTIMATED_ALL_ZERO';
  end if;
  ...
  if exists (
    ...
    raise exception 'MEAL_NUTRITION_MACRO_CALORIE_SANITY_FAILED';
  end if;
  ...
  if exists (
    ...
    where nutrition_status is null
       or nutrition_status not in (
         'approved',
         'missing_source_data',
         'estimated_from_ingredients'
       )
  ) then
    raise exception 'MEAL_NUTRITION_STATUS_UNSUPPORTED';
  end if;

  raise notice 'PASS seed catalog: % recipes / % topics / % chapters',
    v_recipe_count, v_topic_count, v_chapter_count;
end
$$;

-- =============================================================================
-- END FAIL-FAST SEED VALIDATION
-- =============================================================================

commit;

-- =============================================================================
-- BEGIN ROLLBACK-ONLY VIETQR SMOKE
-- =============================================================================

-- Rollback-only local/sandbox smoke for a server-issued VietQR transaction.
-- Run after 01_build_system.sql and 02_seed_data.sql. It creates a temporary request, verifies the
...
-- =============================================================================
-- END ROLLBACK-ONLY VIETQR SMOKE
-- =============================================================================
