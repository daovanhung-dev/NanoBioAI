...
|---:|---|---|---|---|
| 1 | Guest sinh lich trinh AI lan dau sau onboarding | 04, 12 | Chua coding dung DD | `GeneratedPlanService` va main override hien yeu cau Supabase user; guest khong co auth se bo qua sinh plan. |
| 2 | Chan Guest tao lai lich trinh lan 2 theo Q-01 | 04 | Chua coding | Chua thay guard "guest da tao 1 lan" theo thiet bi/local profile/anonymous/account. |
| 3 | V1 allowlist route/use-case cho Guest | 04, 14 | Coding mot phan | AI chat/nutrition/profile co auth guard; dashboard guard dang comment, community va mot so route/use-case chua co guard theo DD. |
...
| 6 | Free quota tao lich trinh 3 lan/thang | 06 | Chua coding | `personal_schedule_quota` chi la planned placeholder; schedule generation hien chi check auth, chua check quota. |
| 7 | Health score theo completion history dung cong thuc DD | 07 | Chua coding theo DD | Co score local/dashboard hien huu, nhung `v2/health_scoring` van planned va Q-05 cong thuc chua chot. |
...
| 13 | Payment event va hoa hong 2 tang | 11 | Chua coding app/backend runtime | Co SQL draft trong `docs/supabase`, nhung Flutter/backend integration va sandbox evidence chua co. |
| 14 | Supabase foundation deploy/verify cho membership/quota/family/sale | 03 | Chua xac nhan coding xong | Co draft SQL, chua thay bang chung da chay sandbox/staging va acceptance/RLS checks pass. |
