...
| M06 `MEMBERSHIP_QUOTA` | Approved | Implemented | Static-verified; Runtime-unverified; Sandbox-unverified | Effective access, trusted quota gateway/RPC, and scheduled membership-expiry reconciliation source exist. |
...
| M06 `MEMBERSHIP_QUOTA` | Approved - DD docs complete | 0 | 100 | 100 | SQL quota RPC contracts, v2 effective access read model, trusted quota gateway, scheduled membership-expiry reconciliation, Free 3/day and 3/month static contract tests, paid bypass tests, Asia/Ho_Chi_Minh period keys, idempotent commit, and no-client-write checks exist. | Supabase sandbox/RLS verification for quota counters/events, membership expiry scheduler, idempotency, reset policy, and client write rejection remains production acceptance backlog. | Apply the canonical rebuild with `pg_cron` in sandbox and execute expiry plus Free, Plus, and FamilyPlus acceptance checks. |
...
| M13 `PAYMENT_MEMBERSHIP` | Approved - DD docs complete | 0 | 100 | 100 | VietQR VCB flow is implemented: server-owned NB reference/memo/recipient config, SQLite-name display boundary, QR/confirm UI, pending_review Admin banner/queue, manual approval/rejection, audit and entitlement activation only on succeeded; focused QR, V2, Admin and SQL-contract tests pass. | Supabase sandbox/RLS migration evidence and bank-app UAT/manual VCB reconciliation remain production acceptance backlog; no bank API/webhook/balance integration is in scope. | Apply migration in sandbox; test customer ownership/idempotency, payments.write Admin review/audit, approval entitlement and rejection; scan QR and reconcile in VCB before release. |
...
| M13 `PAYMENT_MEMBERSHIP` | Supabase sandbox/RLS migration, customer/Admin manual-review flow, entitlement activation/rejection audit, and VietQR bank-app scan/VCB reconciliation evidence. |
