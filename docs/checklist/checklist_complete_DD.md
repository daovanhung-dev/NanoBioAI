...
|---|---|---:|---:|---:|---|---|---|
| M01 `ONBOARDING_PROFILE` | Draft | 2 | 40 | 40 | V1 onboarding local da co page/controller/repository/local datasource va tests. | Guest profile sync, consent, va Supabase contract chua chot theo DD v2.0. | Chot open questions, doi chieu `Overall.md`/`List_Features.md`, sau do harden guest profile persistence va acceptance tests. |
| M02 `PERSONAL_SCHEDULE_AI` | Draft | 1 | 40 | 40 | V1 AI generated plan/lifestyle schedule da co runtime local mot phan. | Quota, guest regeneration guard, ownership, va trusted AI contract chua du ro. | Chot monthly generation rules, noi quota guard truoc AI call, them tests cho guest/free schedule generation. |
...
| M08 `HEALTH_SCORE_HABITS` | Draft | 2 | 40 | 25 | Co dashboard score/local health status va placeholder v2 health scoring. | Q-05/Q-15 formula, weights, skip/miss policy chua chot. | Chot formula, tao domain service official score, map completion history vao score va tests. |
...
| M12 `REFERRAL_DIRECT` | Draft | 5 | 40 | 60 | Sale referral Clean Architecture, Supabase RPC datasource, direct-only rules va tests da co. | Sale approval policy, privacy, anti-fraud, va sandbox verification chua chot. | Verify SQL/RPC in Supabase sandbox, complete participation/referral attach flows va audit tests. |
...
| M14 `SALE_POINTS` | Draft | 8 | 40 | 60 | Co Sale point conversion SQL/RPC draft va sale point ledger UI/data path mot phan. | Point conversion rules, reconciliation, payout, va financial open questions chua chot. | Chot Q-02..Q-10/Q-13, verify conversion RPC, add admin review and ledger acceptance tests. |
| M15 `ADMIN_DASHBOARD` | Draft | 2 | 40 | 60 | Admin app surface, router, dashboard shell, Supabase RPC datasource va contract tests da co. | Admin role matrix Q-12, privacy Q-18, va SQL sandbox verification chua xong. | Chot role matrix, verify Admin SQL in sandbox, harden dashboard permission/error states. |
| M16 `ADMIN_OPS` | Draft | 4 | 40 | 60 | Admin sections users/payments/sales/plans/reports/audit/config va mutation command path da co. | Permission matrix, reason policy, audit completeness, va operation-specific contracts chua approved. | Chot each admin operation contract, add section-level tests va sandbox RLS/RPC checks. |
...
|---|---|---|---|
| M01 `ONBOARDING_PROFILE` | `lib/app_versions/v1/features/onboarding/`, `test/app_versions/v1/features/onboarding/` | Supabase/profile sync contract, guest one-time policy, DD approval. | 1. Doc `docs/DD/onboarding_profile/Overall.md`. 2. Chot open questions. 3. Tao issue/test cho guest profile persistence va sync boundary. |
| M02 `PERSONAL_SCHEDULE_AI` | `lib/app_versions/v1/services/ai/`, `lib/app_versions/v1/features/lifestyle_schedule/`, generated plan tests. | Quota, idempotency, regeneration guard, trusted AI boundary. | 1. Chot quota dependency M06. 2. Add guard before AI generation. 3. Test guest/free limit and retry behavior. |
...
| M08 `HEALTH_SCORE_HABITS` | Dashboard health status and `lib/app_versions/v2/features/health_scoring/health_scoring.dart`. | Official score formula and habits history model. | 1. Chot Q-05/Q-15. 2. Build official scoring service. 3. Add history-based tests. |
...
| M12 `REFERRAL_DIRECT` | `lib/sale_referral/`, `docs/supabase/12-sale-module-update.sql`, sale tests. | Sandbox RPC evidence, anti-fraud, final Sale approval policy. | 1. Verify RPC locally/sandbox. 2. Complete pending/approved participation UX. 3. Add privacy and audit tests. |
...
| M14 `SALE_POINTS` | Sale point ledger/conversion SQL and sale module data path. | Conversion policy, payout/reconciliation, admin review final contract. | 1. Chot financial questions. 2. Verify conversion queue RPC. 3. Add Admin review tests and ledger invariants. |
| M15 `ADMIN_DASHBOARD` | `lib/app_versions/admin/`, `docs/supabase/11-admin-access-dashboard.sql`, admin tests. | Role matrix, privacy scope, sandbox SQL/RPC verification. | 1. Chot Q-12/Q-18. 2. Verify Admin SQL. 3. Add permission/error-state UI tests. |
| M16 `ADMIN_OPS` | Admin sections and mutation command path in `lib/app_versions/admin/`. | Operation-level permissions, reason policy, audit completeness. | 1. Define each operation contract. 2. Add section-specific controller tests. 3. Verify audit rows per mutation. |
