...
| M05 `AUTH_PROFILE_SYNC` | Approved - DD docs complete | 0 | 100 | 100 | V2 auth, Supabase auth datasource, account security, local-first profile path, SQLite v12 `personal_schedule_ai_requests` sync mapping, snapshot/outbox/migration tests, and offline retry tests exist; DD docs have referral attach timing and timezone. | Guest merge/profile sync cross-device sandbox evidence and live registration-only referral attach smoke remain production acceptance backlog. | Verify guest merge, profile sync across devices, schedule AI request sync, and referral attach policy in Supabase sandbox. |
...
| M05 `AUTH_PROFILE_SYNC` | Guest merge/profile sync cross-device sandbox evidence, schedule AI request cloud sync smoke, and live registration-only referral attach smoke. |
