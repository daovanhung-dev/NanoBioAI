Commit de xuat: test(real-device): ghi nhan case PRE-06

# PRE-06

- Trạng thái: PENDING
- Persona/tiền điều kiện: Thu muc evidence
- BD/DD/AC: Evidence gate
- Thiết bị bắt buộc: Xiaomi 220333QPG, Android 11/API 30, 720x1650

## Kịch bản và kết quả mong đợi

Xac nhan quy tac dat ten anh/log hoat dong va anh mau khong co PII.

## Thao tác thực tế

- Chưa thực thi trong chiến dịch 15-07-2026.

## Kết quả thực tế

- Chưa có kết quả quan sát từ điện thoại thật.

## Bằng chứng

- Ảnh điện thoại: chưa có.
- Command/log bổ trợ: chưa có.

## Bug và retest

- Chưa xác định.