# Function List — ADMIN_DASHBOARD / Admin View / Dashboard
...
<a id="admin_dashboard-fn01"></a>
# ADMIN_DASHBOARD-FN01 — loadAdminDashboard

## A. Định danh và trách nhiệm

| Trường | Nội dung |
|---|---|
| Feature cha | ADMIN_DASHBOARD-F01 |
...
| File dự kiến | planned:lib/app_versions/v3/features/admin_dashboard/application/admin_dashboard_fn01.dart |
| Hàm export / endpoint | execute(command, actorContext) hoặc API contract tương ứng |
| Mục tiêu duy nhất | Admin xem chỉ số vận hành theo permission. |
| Không chịu trách nhiệm | Không tự chốt product questions; không truy cập trực tiếp UI hoặc storage ngoài layer được phép. |
| Được gọi bởi | ADMIN_DASHBOARD-V01 hoặc event/API source trong BD section 11.2, AC-19, UC-20 |
...
3. Tải entity liên quan: @{Id=dashboard_metric; Name=Dashboard Metric; Purpose=Chỉ số tổng hợp; Attributes=metric key, period, value, scope; Relationships=Derived from reporting}, @{Id=admin_scope; Name=Admin Scope; Purpose=Phạm vi quyền xem; Attributes=role, permission, scope; Relationships=Limits metrics}.
4. Áp dụng ADMIN_DASHBOARD-BR01 và các rule cross-module từ BD sections 14, 15.
...
# ADMIN_DASHBOARD-FN02 — openAdminDrilldown

## A. Định danh và trách nhiệm

| Trường | Nội dung |
|---|---|
| Feature cha | ADMIN_DASHBOARD-F02 |
| Layer | Use case / Service, called by controller/provider/API handler |
| Loại thực thi | Sync for validation and state read; async/job only when BD requires background processing |
| File dự kiến | planned:lib/app_versions/v3/features/admin_dashboard/application/admin_dashboard_fn02.dart |
| Hàm export / endpoint | execute(command, actorContext) hoặc API contract tương ứng |
| Mục tiêu duy nhất | Admin đi từ chỉ số sang module chi tiết đúng quyền. |
| Không chịu trách nhiệm | Không tự chốt product questions; không truy cập trực tiếp UI hoặc storage ngoài layer được phép. |
| Được gọi bởi | ADMIN_DASHBOARD-V02 hoặc event/API source trong BD section 11.2 workflow |
| Gọi tiếp | Repository/datasource/service planned trong Import_File.md |
...
2. Xác thực actor, role, package entitlement, Sale/Admin scope nếu có.
3. Tải entity liên quan: @{Id=dashboard_metric; Name=Dashboard Metric; Purpose=Chỉ số tổng hợp; Attributes=metric key, period, value, scope; Relationships=Derived from reporting}, @{Id=admin_scope; Name=Admin Scope; Purpose=Phạm vi quyền xem; Attributes=role, permission, scope; Relationships=Limits metrics}.
4. Áp dụng ADMIN_DASHBOARD-BR02 và các rule cross-module từ BD sections 14, 15.
5. Thực thi transaction/idempotency: Conditional - read-only flow does not need transaction; writes still require idempotency and audit.
6. Ghi audit nếu có tác động quyền, tiền, điểm, cấu hình, dữ liệu gia đình hoặc export.
7. Trả Result chuẩn hóa, không trả raw stack trace, raw payment evidence, secret, hoặc health PII không cần thiết.
