...
| Microphone/audio | Microphone may be active only for Sleep Safety; audio handling must be confirmed | No audio provider is intended by the Flutter contract | User-enabled safety monitoring | Feature-optional | Runtime monitoring; confirm whether any derived signal is persisted | Permission + foreground service; device test/policy review open | `SleepSafetyForegroundService`; manual gate open |
...
No client-held Gemini credential is collected or shipped. The final form must
still name the actual third parties and processing/retention terms configured
for the deployed Edge Function.
